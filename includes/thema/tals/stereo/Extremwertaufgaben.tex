...
Am Beispiel einer Himmelslaterne\footnote{Mit «Himmelslaternen» meine ich hier diese selbstgebastelten Heißluftbalone aus Seidenpapier, die mit Spiritus betrieben werden.} soll gezeigt werden, wie dessen Oberfläche unter einer Nebenbedinung (zur Verfügung steht nur eine begrenzte Länge an Holz für die Holzstangen) maximiert wird.

Problemstellung. Eine quaderförmige Himmelslaterne (mit quadratischer Grundfläche) ist unten offen und an Decke und an den vier Seitenwänden mit Seidenpapier augefüllt. Alle zwölf Kanten sind dabei mit einem festen Holzgestell stabilisiert, wobei wir diesmal nur eine acht Meter lange Holzleiste zur Verfügung haben, die wir in die zwölf Kanten aufteilen müssen.
...
\subsubsection{Rezept}
  ...
  \item \textbf{Zielfunktion} definieren. Unsere Zielgröße $F(x)$ kann nun vollständig durch $x$ definiert werden, indem wir die Nebenbedingung $h=2-2x$ in die ursprünglich definierte Zielgröße einsetzen:\
  ...
  \item \textbf{Maximieren/Minimieren} Die Zielfunktion ist hier eine quadratische Funktion, deren Maximum einfach mit Taschenrechner oder Scheitelpunkt bestimmt werden kann. Ich zeige hier die
    ...
    \TNT{3.2}{$$F(x) = -7x^2 + 8x$$\vspace{2cm}}

    Diese Funktion wird maximiert, indem das $x_S$ des Scheitelpunktes $S=(x_s|y_S)$ besitmmt wird. (Wir erinnern uns: $x_S = \frac{-b}{2a}$.) Somit ist hier $x_S = \frac{-8}{2\cdot{}(-7)} = \frac47$. Dies ist gleichzeitig auch unser gesuchtes $x$.
