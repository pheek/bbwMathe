...
\begin{gesetz}{}{}
$\frac{1}{a^{-n}} = \left(\frac{1}{a}\right)^{-n}=a^n$
\end{gesetz}
Begründung:
\TNT{4}{
\GESO{Beispiel. Nach Definition gilt:

$$\left(\frac1{10}\right)^{-3}  = \frac1{\left(\frac1{10}\right)^3}  = \frac1{10^{-3}} = \frac1{0.001} = 1000 = 10^3$$

\vspace{2cm}
}%% end GESO
\TALS{
Es gilt für alle $a$ (nach Definition):
$$a^{-n} = \left(\frac1a\right)^n$$
Somit gilt das auch anstelle von $a$ für $\frac1b$:

$$\left(\frac1b\right)^{-n} = \left(\frac1{\frac1b}\right)^n = b^n$$
}%% END TALS
}%% END TNT
...
\TNT{2.4}{
$$t = \frac{s}v = \frac{12[ \textrm{m}]}{3 \frac{[\textrm{cm}]}{[\textrm{s}]}} =  \frac{4[ \textrm{m}]}{1 \frac{[\textrm{cm}]}{[\textrm{s}]}} =
 \frac{4[ \textrm{m}]}{10^{-2}\frac{[\textrm{m}]}{[\textrm{s}]}} = 4\cdot{} 10^2 [\textrm{s}] = 400 [\textrm{s}]\approx 6-7 Min.$$
}%% END TNT
