...
\TNT{12}{Ansatz: $f(t) = f(0) \cdot a^{\frac{t}{\tau}}$ mit $\tau = 1$. $\tau$ ist immer = 1, wenn der Wachstumsfaktor der charakteristischen Zeit 1 (hier 1 Jahr) entspricht. Also

  $f(t) = f(0) \cdot a^t$.

  Dabei ist $f(0)$ der Anfangsbestand.

  Der Wachstumsfaktor ist 1.04 (= 100\% + 4\% = 1 + $\frac{4}{100}$).

  Somit ist der Bestand nach $t$ Jahren $f(t) = f(0) \cdot 1.04^{t}$.

  Die Bevölkerung hat sich verdoppelt, wenn $f(t) = f(0) \cdot 2$, also wenn $1.04^t = 2$ ist.

  Diese Exponentialgleichung kann durch Logarithmieren gelöst werden:

  $$1.04^t = 2$$
  $$\ln(1.04^t) = \ln(2)$$
  $$t\cdot{}\ln(1.04) = \ln(2)$$
  $$t = \frac{\ln(2)}{\ln(1.04)}\approx{17.7[Jahre]}$$

  Voraussichtlich verdoppelt sich (bei ungebremstem Wachstum) die Bevölkerung von Irland innerhalb von 17 bis 18 Jahren.

  BESSERES BEISPIEL, wo $\tau$ auch eine Rolle spielt!

  1990 hatte Irland 3.5 Mio EW, 2019 waren es 4.9 Mio. Was prognoszieren Sie füxr 2025 (lsg. 5.25 Mio)?
}%% END Trainer
\newpage
...
\item Wie sieht es mit der Basis $e$ aus? Mit welchem $b$ kann man die
  Zunahme als $f(t) = e^{b\cdot{}t}$ angeben?
\end{enumerate}
...
$\ln(2^\frac15) = b\cdot \ln(e) = b\cdot 1 = b \approx 0.1386$
}%% END TNT
