%%
%% 2019 07 04 Ph. G. Freimann
%%

\subsection{Ähnlichkeit von Figuren}\index{Aenlichkeit@Ähnlichkeit}
\sectuntertitel{Treffen sich zwei Parallele...}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Lernziele}

\begin{itemize}
\item Streckungsfaktor (Ähnlichkeitsfaktor)
\item Translation
\item Drehung
\item Streckung
\item Spiegelung
%%\item Scherung (Lernziel ???)
\end{itemize}
\TadBMTG{72}{5.3}

\newpage

\subsubsection{Streckungsfaktor}\index{Streckungsfaktor}

\bbwCenterGraphic{10cm}{tals/plani/img/aehnlichkeit.png}

%%\raisebox{-2cm}{\includegraphics[width=11cm]{tals/plani/img/aehnlichkeit.png}}

\begin{definition}{Ähnliche Figuren}{}
  
Proportionalität: Sind zwei Figuren ähnlich, so existiert ein
\textbf{Streckungsfaktor}\index{Streckungsfaktor!Planimetrie}\footnote{Der
  Streckungsfaktor heißt auch Ähnlichkeitsfaktor\index{Aenlichkeitsfaktor@Ähnlichkeitsfaktor!Planimetrie}.}
$k$, der die Strecken ($a$, $b$, $c$, ...) der Originalfigur
($\mathbb{P}$) auf die Strecken ($a'$, $b'$, $c'$, ...) der Bildfigur
($\mathbb{R}$) «abbildet»:

$$a' = k\cdot{}a; b' = k\cdot{}b; c' = k\cdot{} c; ...$$

Winkel: Zudem stimmen die Figuren in allen entsprechenden Winkel überein.
\end{definition}

Daraus ergibt sich durch Division:


\begin{gesetz}{Streckungsfaktor\index{Streckungsfaktor!Planimetrie}
    = Ähnlichkeitsfaktor\index{Aenlichkeitsfaktor@Ähnlichkeitsfaktor!Planimetrie}}{}

  Für den Streckungsfaktor $k$ gilt:
  $$k = \frac{a'}{a} = \frac{b'}{b} = \frac{c'}{c} = ...$$
\end{gesetz}

\newpage

\subsubsection{Flächen ähnlicher Figuren}\index{Flache@Fläche!ahnliche@ähnliche Figuren}
Für die Flächen $A$ (Figur $\mathbb{P}$) und $A'$ (Figur $\mathbb{R}$) gilt jedoch

\begin{gesetz}{Flächen ähnlicher Figuren}{}
  Mit dem Streckungsfaktor $k \left(=\frac{a'}{a} = \frac{b'}{b}= ...\right)$ gilt für die
  Flächen ähnlicher Figuren:
  
  $$A' = k^2 \cdot{} A  \text{ bzw. } k = \sqrt{\frac{A'}{A}}$$
\end{gesetz}


\subsection*{Aufgaben}
%%\TALSAadBFWG{64ff (zentrische Streckung)}{257. 264. 271. (berechnen)}

Ähnliche Figuren:
\TALSAadBMTG{83}{22. [Tipp: Sind die beiden Dreiecke ähnlich?]}

%%\TALSAadBFWG{69ff (ähnliche Figuren)}{279. 283. 286. 290.}
Fläche:
\TALSAadBMTG{83ff}{23., 25., 27., 29., 30., 40.}

Kreise:
\TALSAadBMTG{85}{41. und 42.}

%%\TALSAadBFWG{73ff (ähnliche Dreiecke)}{306. 309. 319.}

Strukturaufgaben
\aufgabenFarbe{Strukturaufgaben (GLF): 3. y) bb)}

%%\TALSAadBFWG{81ff (Ähnlichkeit am Kreis)}{338. 340. 341. 342.}

\GESOAadBMTA{???}{???}

