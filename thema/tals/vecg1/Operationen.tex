...
\TALSAadBMTG{245ff}{5. d) e) f), 6. a) c), 7.}
...
Welche Spezialfälle kommen Ihnen in den Sinn, wodurch eine solche Zerlegung
nicht eindeutig möglich ist?

\TNT{15.2}{
Die Zerlegung ist nicht eindeutig möglich, wenn $\vec{a}$ parallel zu $\vec{b}$
ist.

1. Fall $\vec{c}$ ist nicht parallel zu $\vec{a}$ (und $\vec{b}$),
dann gibt es keine Zerlegung.

2. Fall $\vec{c}$ ist auch parallel zu $\vec{a}$ (und $\vec{b}$), dann
gibt es unendlich viele Zergelungsmöglichkeiten.
}
