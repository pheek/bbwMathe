...
\subsection*{Aufgaben}
\TALSGeomAadB{179}{5., Nr. 6. Teile (1) (3) (5) und (6) und Nr. 7. a) d)}
...
\begin{bemerkung}{}{}
  $$\vec{a\vphantom{x}} + \vec{b} = \vec{a\vphantom{b}} \Longleftrightarrow \vec{x\vphantom{b}} = \vec{a\vphantom{b}} - \vec{b}$$
\end{bemerkung}

Gemoetrisch: $\vec{a\vphantom{b}} - \vec{b} = \vec{a\vphantom{b}} +
(-\vec{b})$.
...
Grafik: Es gilt: $\vec{x} + \vec{b} = \vec{a}$ und das heißt $\vec{x}
= \vec{a} - \vec{b}$.
}%% END TNT

\subsection*{Aufgaben}
\TALSGeomAadB{179ff}{6. (2) und (4), 7. b) c) 9. und 10. a) c) i)}
\newpage
...
\draw [->,blue] (1,2) -> (2.5,3.999);
\bbwLetter{6.5,3}{\vec{c}}{ForestGreen}
\draw [->,ForestGreen] (6,2) -- (6,4.5);
}%% END bbwGraph

In obigem Beispiel ist

\TNT{5.2}{
	$\frac38 \vec{a\vphantom{b}} + \frac45 \vec{c}  = \vec{b}$
  
	und somit

	$\frac45 \vec{c} = -\frac38 \vec{a\vphantom{b}} + \vec{b}$

  Mittels Division mit $\frac45$ erhalten wir $\vec{c}$:

  $\vec{c} = -\frac{15}{32}\vec{a\vphantom{b}} + \frac54 \vec{b}$
...
\TALSGeomAadB{180ff}{12. b) c), 13., 16. b) d) e) und 17. a) b)}
