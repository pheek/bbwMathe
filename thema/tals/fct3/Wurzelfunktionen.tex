...
Die geforderte Länge $l$ (in m) eines Pendels lässt sich wie folgt annähern:

$$l = g \cdot{} \left(\frac{t}{2\pi}\right)^2$$


Dabei ist $t$ die gemessene Schwingungsdauer in
...
\begin{gesetz}{Scheitelform}{}
  Bei der Funktion
  $$f: y = \LoesungsRaumLen{50mm}{a\cdot{}\sqrt{\pm(x-X_S)} + Y_S}$$
liegt der \textit{Scheitelpunkt} bei $S=(X_S | Y_S)$ und der Formfaktor $a$ kann
...
\TNTeop{
  Ansatz: $y = a\cdot{}\sqrt{\pm(x-X_S)} + Y_S = a \sqrt{-(x-(-4))} + 2 = a \sqrt{-x-4} + 2$
...
\TNTeop{
  ...
  $j: y=2\cdot{} \sqrt{-(x-4)} + 3 = 2\cdot{} \sqrt{4-x} + 3$

  $k: y=\frac{-1}2 \cdot{} \sqrt{-(x+1)} + 8$
}%% end TNT
...
\TALSAadBMTA{310}{40., 43. g) , 45. a) und optional 44. b)}
