...
\begin{gesetz}{gerade Exonenten}{}
  ...
  Begründung: Für $i\in\mathbb{Z}$ gilt:
...
\aufgabenFarbe{Zeichnen Sie Graphen obiger beiden Beispiele mit dem
  Taschenrechner:
...
\olatLinkTALSStrukturaufgabenSPF{Teil 2 (mit TR)}{15}{48.}
...
\begin{definition}{Linearfaktor}{}
  Ein Term in $x$ der Form
  $$T(x) = (x+a)$$
  mit $a\in\mathbb{R}$ heißt Linearfaktor in $x$.
\end{definition}
...
Bei Polynomfunktionen reicht es, auf den Summanden mit dem höchsten
Exponenten zu schauen, um den Grobverlauf des Graphen voraussagen zu
können (sei $a > 0$):
...
\olatLinkTALSStrukturaufgabenSPF{Teil 2 [mit
    Taschenrechner]}{16}{51. (erst die Nullstellen bestimmen) und 52.}
...
\TNTeop{Welche Information fehlt noch? ``berührt'' ist nicht gleich
  ...
  Danach ein Gleichungssystem mit den gegebenen Punkten erstellen

  Lösung:

  $$b=-4, c=-10, d=20, a=\frac{1}{50}$$
...
\TRAINER{  \bbwCenterGraphic{70mm}{tals/fct3/img/SechsLinearfaktoren.png}}
  ...
  \item Klammeransatz $f(x) =
    \LoesungsRaumLang{a\cdot{}(x-\Box)(x-\Box)(x-\Box)(x-\Box)(x-\Box)(x-\Box)}$
  \item Nullstellen (inkl. doppelte\footnote{Doppelte Nullstellen heißen auch
    ...
    $$f(x) = \LoesungsRaumLang{a\cdot{}(x+1)(x-1)(x-1)(x-3)(x-3)(x-3)}$$
  ...
  \item Lösung: $f(x) = \LoesungsRaumLang{0.1 (x+1)(x-1)^2(x-3)^3}$ 
    \end{enumerate}
...
\olatLinkTALSStrukturaufgabenSPF{Teil 1 [ohne TR]}{6ff}{6. und 10.}
