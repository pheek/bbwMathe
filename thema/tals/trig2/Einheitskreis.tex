...
\begin{gesetz}{Tangens}{}
  $$\tan(\varphi) = t = \frac{t}{1} =  \frac{s}{c} = \frac{\sin(\varphi)}{\cos(\varphi)}$$
\end{gesetz}
\newpage
...
\TNTeop{
  am besten am Einheitskreis ablesen. Ansonsten rchnerisch:
  
  1. Sinus ist ungerade (gesetz) $$\cos(x) = \sin(-211\degre)$$
...
Der $\cos$ ist an der $x$-Achse gespiegelt (gerade), somit ist
