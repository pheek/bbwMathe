...
\subsection{Periodische Lösungen einschränken}\index{periodische Lösungen!Taschenrechner}
...
\TNTeop{
  ...
  b) Füge dem Gleichungssystem $T(x)=14$ als Gleichung hinzu und schränke den Definitionsbereich mit $|$ ein:
    $$\text{\texttt{solve}}(\{T(81)=12;T(249)=13;T(x)=14\},\{a,c,x\})|x>0 \text{ AND } x<365$$
...
\TNTeop{
  ...
  rad: $\varphi = \arctan\left(\frac{3\cdot{}(\sin(9)-\sin(1))}4\right) \approx -0.3115 \text{ rad }$
  ...
  \textbf{c)} Die Nullstellen werden typischerweise mit $\text{\texttt{solve}} (f(x)=0, x)$ gelöst:

  $$x_1\approx{} 0.6416 \text{ und } x_2\approx{} 2.212$$
  ...
  $$p(x) := a\cdot{}(x-0.6416)\cdot{}(x-2.212); \text{\texttt{solve}} (p(0)=10; a); a\approx{}7.045$$
}%% END TNTeop
%%\newpage
