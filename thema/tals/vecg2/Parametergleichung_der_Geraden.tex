...
Erinnerung: Wie hatten wir bisher eine Gerade beschrieben?

\TNT{4}{
  $f:\hspace{10mm} y=ax+b$
}
...
gegebener Punkt. So ist
 $$\overrightarrow{OP} = \overrightarrow{v_P} = \vec{p} = \noTRAINER{\hspace{15mm}} \TRAINER{\Spvek{4;-3;s}}$$
...
\begin{bemerkung}{Parameter $t$}{}
  Für jedes $t\in\mathbb{R}$ wird mit
  $$\overrightarrow{OA} + t\cdot{} \vec{u} = \vec{a} + t\cdot{} \vec{u}$$
  ein weiterer Punkt auf der Geraden bestimmt. Der Vektor $\overrightarrow{OA}$ wird
  auch \textbf{Stützvektor}\index{Stützvektor} genannt; der Vektor $\vec{u}$ wird auch \textbf{Richtungsvektor}\index{Richtungsvektor} genannt.
\end{bemerkung}

\begin{bemerkung}{Lineare Funktion}{}
  Mit dieser Methode ist es, im Gegensatz zu linearen Funktionen, möglich, Geraden als Funktionen zu beschreiben, die parallel zur $y$-Achse liegen.
  \end{bemerkung}
\newpage
...
\begin{definition}{Parametergleichung}{}
  ...
  oder kurz (mit $\vec{a} = \overrightarrow{a_A}$):
...
\olatLinkArbeitsblatt{Geradengleichung}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/110262782958559}{Kap. 1: Aufg 1., 2., 3., 4. $g_1$, 5. a) b) und 6. a)}
