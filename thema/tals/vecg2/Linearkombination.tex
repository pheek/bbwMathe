...
\begin{definition}{Linearkombination}{}
 ...
 %% gilt für jede beliebige Anzahl von Summanden:
  ...
  x_3\cdot{}\vec{a_3} + ...$$
  eine \textbf{Linearkombination} der gegebenen Vektoren $\vec{a_1}$, $\vec{a_2}$, $\vec{a_3}$, ... 
\end{definition}


\begin{beispiel}{Linearkombination}{}
Oft wollen wir einen gegebenen Vektor $\vec{g}$ durch eine Linearkombination von
anderen gegebenen Vektoren ausdrücken.

Gegeben:$\vec{g}$ und die Vektoren $\vec{a\vphantom{b}}$, $\vec{b}$ und $\vec{c\vphantom{b}}$

Gesucht $r$, $s$ und $t$ in $\mathbb{R}$, sodass gilt:

$$\vec{g} = r\cdot{}\vec{a\vphantom{b}} + s\cdot{}\vec{b} + t\cdot{} \vec{c\vphantom{b}}$$
...
\begin{beispiel}{Linearkombination}{}
  ...
  \TNT{4}{\gleichungZZ{3}{7s + 4t}{5}{9s + 3t}}%% end TNT
...
\olatLinkArbeitsblatt{Vecg II
  ...
  Hand: 22. und 24. Mit TR: 23.}%% END

\TRAINER{Im Aufgabendokument noch nachtragen: Von Hand: 22/24/25 mit
  TR: 23}

\newpage
\subsection{Lineare Abhängigkeit}\index{linear abhängig}\index{Abhängigkeit!lineare}

Ich brauche zwei Vektoren, um eine Ebene und drei Vektoren, um den
Raum aufzuspannen.
...
\begin{definition}{Linear abhängig}{}
  Ein Vektor $\vec{b}\ne \vec{0}$ heißt von den Vektoren $\vec{a}$,
  $\vec{b}$ und $\vec{c}$ \textbf{linear abhängig}, wenn es eine
  Linearkombination mit $r$, $s$ und $t \in \mathbb{R}$ gibt, für die gilt:

  $$\vec{b} = r\cdot{}\vec{a\vphantom{b}} + s\cdot{}\vec{b} + t\cdot{}\vec{c\vphantom{b}}$$
  \end{definition}
\newpage
...
\olatLinkArbeitsblatt{Vecg II
  Komponenten}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/108600437225393}{Von Hand: 26. bis 29.}%% end
