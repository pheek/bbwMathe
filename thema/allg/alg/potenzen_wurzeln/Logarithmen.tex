...
\begin{gesetz}{Basiswechsel}{}\index{Basiswechsel!Logarithmen}
  ...
  $$\log_a(p) =  \LoesungsRaumLen{30mm}{\frac{\lg(p)}{\lg(a)}}  \TALS{ =
    \LoesungsRaumLen{30mm}{\frac{\log_b(p)}{\log_b(a)}}}$$

  und somit

  $$a^x=b \Longleftrightarrow x= \LoesungsRaumLen{25mm}{\frac{\lg(b)}{\lg(a)}} = \LoesungsRaumLen{25mm}{\log_a(b)}$$
\end{gesetz}
...
\TALS{
  ...
  \end{gesetz}
  \TNT{3.2}{Beweis: $$a^u\cdot{}a^v = a^{u+v}$$
    logarithmieren:
    $$\log_a(a^u\cdot{}a^v) = \log_a{a^{u+v} = u+v = \log_a(a^u) + \log_a(a^v)}$$
    Setze $x=a^u$ und $y=a^v$:
    $$\log_a(x\cdot{}y)  = \log_a(x) + \log_a(y)$$
  }
    \begin{gesetz}{Quotientenregel}{}
  $$\log_a\left(\frac{u}{v}\right)= \LoesungsRaumLen{50mm}{\log_a(u) - \log_a(v)}$$
  \end{gesetz}%%
    \TNT{3.2}{Beweis s. analog}
    \newpage
} %% END TALS
