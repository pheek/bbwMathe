...
a) Wir können bereits $$x^5=32$$ lösen, indem wir beidseitig die
5. Wurzel ziehen:

$$x = \LoesungsRaum{\sqrt[5]{32}} = \LoesungsRaum{\sqrt[5]{2^5}} = \LoesungsRaum{2}$$

b) Ebenso können wir Probleme mit der Unbekannten im Exponenten lösen,
...
\DeclareRobustCommand{\tempA}{6. (ohne TR)}

\GESO{
  \olatLinkArbeitsblatt{A2Log}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/110436259154906}{\tempA}
}%% end GESO
\TALS{
  \olatLinkArbeitsblatt{A2Log}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/110436259219039}{\tempA}
}%% ent TALS
...
Vieles ist mit dem \textbf{Taschenrechner} lösbar. Denken Sie jedoch immer
...
\begin{beispiel}{Taschenrechner}{}
  ...
  \TNT{3.2}{TR: $\log_{\Box}(\Box) \approx 2.837$}
  \TRAINER{\vspace{15mm}}
\end{beispiel}

\subsection*{Aufgaben}

\DeclareRobustCommand{\tempA}{7. (mit TR)}
...
\TALS{
  ...
  \begin{gesetz}{Der Logarithmus ist der Exponent}{logIstExponent}
    $$a^{\log_a(p)} = \LoesungsRaumLen{20mm}{p}$$
  \end{gesetz}
}%% END TALS
\newpage
\subsection*{Aufgaben}
...
\TALS{
  \olatLinkArbeitsblatt{A2Log}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/110436259219039}{13. und
    14. (auch challenges)}
}%% ent TALS
...
\begin{beispiel}{Power-Funktion}{}
  ...
  denn es gilt:
  $$x = \e^{\ln(x)} = \e^{\ln\left(5.3^{7.2}\right)} = \e^{7.2\cdot{}\ln(5.3)}$$
...
Beispiel (optional)
\TNT{5.2}{
aus  $a^x=v \Longleftrightarrow \left(\frac{1}{a}\right)^x = \frac{1}{v}$

folgt $\log_a(v) = \log_{\frac{1}{a}}\left(\frac{1}{v}\right)$

Sirup Beispiel (Wann ist genau 1 \% der alten Luft noch da?)

$$\left(\frac{1}{\e}\right)^n = 0.01 \Longrightarrow n =
\log_{\frac{1}{e}}\left(\frac{1}{100}\right) \Longleftrightarrow
n=\ln(100)\approx 4.6$$
}%% end TNT
