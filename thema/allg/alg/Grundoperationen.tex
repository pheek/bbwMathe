%%
%%
\newpage
\section{Grundoperationen}\index{Grundoperationen}
\renewcommand{\bbwAufgabenBlockID}{A1G}
\sectuntertitel{Ein Sandhaufen plus ein Sandhaufen ist immer noch \textbf{ein} Sandhaufen!}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Lernziele}
\begin{itemize}
\item Addition\index{Addition}, Subtraktion\index{Subtraktion}
\item Multiplikation\index{Multiplikation}, Ausmultiplizieren\index{ausmultiplizieren|textbf}
\item Distributivgesetz\index{Distributivgesetz},
  Assoziativgesetz\index{Assoziativgesetz},
  Kommutativgesetz\index{Kommutativgesetz}
\end{itemize}

%%\TALSTadBFWA{15}{1.2}
\TadBMTA{28}{2}
%%\TALSTadBMTA{28}{2}
\newpage

\subsection{Addition und Subtraktion}\index{Addition|textbf}\index{Subtraktion|textbf}
\begin{beispiel}{}{}
%%  $$x^2-((x^3-x^2)-(-(-x+x^2)-(x^2-x^3)))$$
$$-bx-(-bx^2+((xb-2ax)-(2bx-x^2b))+3ax)$$
\end{beispiel}

\TNT{10}{\bbwCenterGraphic{17cm}{allg/alg/img/KlammernLoesen.png}}%% END TNT

\begin{rezept}{Addition und Subtraktion}{}
\begin{itemize}
\item Es können nur gleiche Variable (bzw. Produkte von Faktoren)
      addiert (bzw. subtrahiert) werden:

      $7ab^2 +2a^2b+4b^2a = \LoesungsRaumLen{60mm}{11ab^2 + 2a^2b}$.
\item Klammern werden von innen nach außen aufgelöst.
\item Negative Vorzeichen vor Klammern wechseln die Vorzeichen der
      Summanden innerhalb der Klammer.
\item Sequenzen mit Subtraktionen werden von links
      nach rechts geklammert: $5-3+1 = \LoesungsRaumLen{40mm}{(5-3) + 1}$
%%\item Gleichwertige Operationen\footnote{Zum Beispiel alles Minus und Plus oder aber zum Beispiel alles \textit{Punkt}-Operationen (Produkt/Quotient).} werden von links nach rechts \textit{geklammert}:
%%  $$10-4+5 = (10-4) + 5 \ne 10-(4+5)$$
\end{itemize}
\end{rezept}
 \newpage


\begin{gesetz}{Assoziativ- und Kommutativgesetz}{}
 \begin{itemize}
  \item Assoziativgesetz: \LoesungsRaumLen{50mm}{$a+(b+c) = (a+b)+c$}
  \item Kommutativgesetz\footnote{lat. \textit{commuto} = tauschen}:
  \LoesungsRaumLen{50mm}{$a + b = b + a$}
 \end{itemize}
\end{gesetz}


\subsection*{Aufgaben}
%%\GESOAadBMTA{34}{1. a) c) e) g), 2. a) e), 3. b), 5. a) f) und 6. d) h)}
%%\TALSAadBMTA{15}{19.}
\GESO{\olatLinkArbeitsblatt{Grundoperationen [A1G]}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/105796961619595}{1. und 2.}}
\TALS{\olatLinkArbeitsblatt{Grundoperationen [A1G]}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/105796961688601}{1. und 2.}}


\newpage
\subsection{Multiplikation}\index{Multiplikation|textbf}
\TadBMTA{29}{2.2}
%%\TALS{Theorie im Buch \cite{frommenwiler17alg} S. 16 Kap. 1.3}

Für die Addition und die Multiplikation gelten die folgenden
Gesetze:



\begin{gesetz}{Gesetze zu Addition und Multiplikation}{}
\begin{itemize}
\item Assoziativgesetz: \LoesungsRaumLen{50mm}{$a\cdot(b\cdot{}c) = (a\cdot b)\cdot c$}
\item Kommutativgesetz:  \LoesungsRaumLen{50mm}{$a\cdot b = b \cdot a$}
\item Distributivgesetz\footnote{lat. \textit{\textbf{distribuere}} = verteilen}:\\
  $a\cdot (b+c) = \LoesungsRaumLen{50mm}{a\cdot b + a\cdot c}$\\

  \noTRAINER{\vspace{5mm}}
  
  $a\cdot (b-c) = \LoesungsRaumLen{50mm}{a\cdot b - a\cdot c}$\\
%%  $(a+b)\cdot c = ac + bc$\\
%%  $(a-b)\cdot c = ac - bc$\\
%%  $(a+b):c = a:c + b:c$\\
%%  $(a-b):c = a:c - b:c$\\
  
  \end{itemize}
\end{gesetz}

\subsubsection{Kommutativgesetz}
Mathematisch gilt $5\cdot{}4$ = $4\cdot{}5$.


\bbwCenterGraphic{60mm}{allg/alg/img/VierHaende.png}

In der realen Welt (Bild oben) sind jedoch die Maßeinheiten zwingend:

$4 \cdot{} 5 [Finger] = 5 [Finger] \cdot{} 4 \ne 4 [Finger] \cdot{} 5$

Die Gleichheit würde nur gelten, wenn man die Fingerquadrupel (alle
Daumen, alle Zeigefinger, ...) zusammenfassen würde.

Hier noch ein Beispiel, wo das Kommutativgesetz (gerade wegen falschen
Einheiten) definitiv \textbf{nicht} gilt:

$$6 \cdot{} \text{ 12-er Schlüssel}  \ne 12 \cdot{} \text{ 6-er Schlüssel}$$

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555

\subsubsection{Distributivgesetz}

\youtubeLink{https://www.youtube.com/watch?v=HVk43UypIdY}{Distributivgesetz}
\TNTeop{Platz für graphischen Beweis: a(x+y) = ax + ay

\bbwCenterGraphic{100mm}{allg/alg/img/Distributivgesetz.png}
}
\newpage

\subsubsection{Ausmultiplizieren}\index{ausmultiplizieren}
Beim Ausmultiplizieren wird jeder Summand in der Klammer mit dem
Faktor vor (bzw. nach) der Klammer multipliziert.
\begin{beispiel}{}{}

  $4\cdot (x + 5) =\LoesungsRaumLang{4\cdot x + 4\cdot 5 = 4x + 20}$
\end{beispiel}

\begin{beispiel}{}{}
%%  \noTRAINER{$$(x + 7)\cdot(8-y) = x\cdot(8-y) + 7\cdot(8-y) =
%%  8x-xy+56-7y$$}\TRAINER{\bbwCenterGraphic{15cm}{allg/alg/img/DistributivRechts.png}}
%%oder
\TRAINER{\\}
  $$(x + 7)\cdot(8-y) = \LoesungsRaumLang{8x+56-xy-7y}$$

  \TRAINER{\par Jeden
  Summanden der ersten Klammer mit jedem Summanden der zweiten
  Klammer multiplizieren}
\end{beispiel}

\newpage

\subsubsection{Achtung}
Auch wenn die folgenden Ausdrücke sehr ähnlich aussehen, so
handelt es sich beim ersten um eine \textbf{Differenz} und bei den anderen um
ein \textbf{Produkt}\TRAINER{ (Streng genommen um die Gegenzahl eines
Produktes) }!

$$10-(x-4) =\LoesungsRaum{14-x}$$

$$-10(x-4) = \LoesungsRaum{-10x + 40}$$

$$-10(x\cdot{}(-4)) = \LoesungsRaum{40x}$$

Diese Unterschied wird auf dem Taschenrechner besonders gut deutlich:

\tiprobutton{7}\tiprobutton{minus}\tiprobutton{3}  \tiprobutton{enter}  \LoesungsRaumLang{$ 7 - 3 = 4$}

\vspace{3mm}
\tiprobutton{7}\tiprobutton{neg}\tiprobutton{3} \tiprobutton{enter}  \LoesungsRaumLang{$ 7\cdot{}(-3) = -21$}


\subsubsection{Referenzaufgabe}
$$3ab-(x-a(2-b))\cdot{}3$$

\TNTeop{$$3ab-[(x-2a+ab)\cdot{}3]$$
$$3ab - (3x -6a +3ab)$$
$$3ab - 3x +6a -3ab$$
$$6a-3x$$}
\newpage

\subsection*{Aufgaben}
%%\olatLinkArbeitsblatt{Grundoperationen [A1G]}{\GESO{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/105796961619595}\TALS{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/105796961688601}}{3., 4. und 5.}

\DeclareRobustCommand{\tempA}{3., 4. und 5.}

\GESO{\olatLinkArbeitsblatt{Grundoperationen [A1G]}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/105796961619595}{\tempA}}
\TALS{\olatLinkArbeitsblatt{Grundoperationen [A1G]}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/105796961688601}{\tempA}}
%%\aufgabenFarbe{Aufgaben zu Grundoperationen im OLAT Aufg. [A1G] 3., 4. und 5.}
\newpage
