...
\TALS{
  ...
  Mancherorts wird auch die Menge ${0, 1, 2, 3, 4, ...}$ als die Menge
...
\begin{bemerkung}{Subtraktion}{}
  ...
  möglich\footnote{Ebenso ist eine \textbf{Division} nur bedingt
    möglich, dazu aber mehr bei den ganzen und den rationalen Zahlen.}:
  \TNT{4}{
  $$8-4 \in \mathbb{N}$$
  aber:
...
Die Vereinigungsmenge der natürlichen ($\mathbb{N}$), der Null
und der negativen Zahlen nennen wir die \textbf{ganzen} Zahlen und
bezeichnen die Menge mit $\mathbb{Z}$.
  \end{definition}
...
\begin{itemize}
  ...
  \item \LoesungsRaumLang{\textbf{Subtraktion} ($-$)}\TRAINER{kein Lernziel: [Minuend - Subtrahend = Differenz]}
  \end{itemize}
...
\TNT{2.8}{$$\frac{A}{B} = \frac{\text{Zähler}}{\text{Nenner}} := (A)
  : (B)$$}
...
\TNT{2}{$$\frac{1}{9} = 0.\overline{1}, \frac{1}{99} = 0.\overline{01}, ..., \frac{1}{99999} =
  0.\overline{00001}$$

Daher ist}%% end TNT


$$0.\overline{13} = 0.131313... = \LoesungsRaumLang{13\cdot{}\frac1{99}=\frac{13}{99}}$$

%\TNTeop{Bem.: $\frac{1}{9} = 0.111\overline{1}$, $\frac{1}{99} =
%0.0101\overline{01}$, $\frac{1}{999} = 0.001001\overline{001}$, ...}
...
\end{bbwFillInTabular} 
  ...
  \TNTeop{
    ...
    möglich repetitiv eingegeben. Achtung, dies geht jedoch nicht immer!
