...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Beispiel: (gebrochen) rationale Funktion\GESO{ (optional)}}
...
Polynomfunktionen und rationale Funktionen sind «\textit{weiche}» Funktionen, die jedoch \textbf{Polstellen}\index{Polstelle} (hier bei $x=0$) und Asymptoten\index{Asymptote} aufweisen können.
...
mit $z \in \mathbb{Z}\backslash\{0\}$\TRAINER{ $z=0$ ist eine
  Konstante Funktion; $z=1$ ist eine 
  Proportionalität.}.
...
\begin{bbwFillInTabular}{|c|c||c|c|}\hline
  ...
  $4$ & \TRAINER{$\frac83$}         &  $-4$  & \TRAINER{$\frac83$} \\\hline
...
\begin{definition}{Parabel}{definition_parabel}
  Ist $n\in \mathbb{N}$ mit $n\ge 2$, so wird der Graph der Potenzfunktion
  $$x\mapsto a\cdot{}x^n$$
...
Stefan-Boltzmannsches T-hoch-vier-Gesetz\index{Boltzmann!$T^4$-Gesetz}

\leserluft{}

Die Funktion $P(T) = \sigma\cdot\varepsilon\cdot
  A\cdot{}T^4$ beschreibt, welche Strahlungsleistung $P$ (in Watt) ein Körper der
  Oberfläche $A$ (in $ \text{m}^2$) bei Temperatur $T$ (in
  $\degre$Kelvin $= {}\degre\-\text{C}+273.15$) aussendet.

  $\sigma$ = Stefan-Bolzmann-Konstante = $5.67\cdot{}10^{-8} \,\, \frac{\text{W}}{\text{m}^2\cdot{} \text{K}^4}$
...
\TNTeop{$T$ = 20 Grad Zimmertemperatur in Kelvin: 293.15;
  Brick (Haus) hat Emissionswert ca. 0.75; Haus Oberfläche $600 \text{m}^2$

  $$P(293.15) \approx 5.67\cdot{}10^{-8} \cdot{} 0.75 \cdot{} 600
  \cdot{} 293.15^4 \approx 188 \text{kW} $$
  ...
  $$P(295.15) \approx 5.67\cdot{}10^{-8} \cdot{} 0.75 \cdot{} 600
  \cdot{} 295.15^4 \approx 194 \text{kW} $$

  Dies entspricht einer Differenz von ca. 5 kW für nur zwei Grad wärmer
  im Haus!

  \vspace{50mm}
  
  Idee: Bei TALS auch das \weblink{Hagen-Poiseuille-Gesetz}{https://de.wikipedia.org/wiki/Gesetz\_von\_Hagen-Poiseuille}
...
\TNTeop{
  großer Exponent: Der Graph wird ``eckig''

  gerader Exponent: Der Graph ist spiegelsymmetrisch an der $y$-Achse

  ungerader Exponent: Der Graph ist punktsymmetrisch
  (bzw. drehsymmetrisch) am Ursprung
...
\begin{tabular}{c|p{8cm}}
  $x^2$ & \vspace{0.1mm}\TNT{0.8}{An der $y$-Achse: Achsensymmetrisch}\\\hline
  $x^3$ & \vspace{0.1mm}\TNT{0.8}{Am Ursprung $O(0|0)$: Punktsymmetrisch}\\\hline
...
\TNTeop{ Punkte einsetzen in die Funktionsgleichung:
  ...
  Weil aber $\frac{8192}{1458} > 0$ ist, folgt, dass $k$ gerade ist.
  
  Und weil nun $k$ gerade ist, folgt dass $\left(\frac{-4}3\right)^k
  = \left(\frac43\right)^k$.

  Daraus ergibt sich
  $$\frac{8192}{1458} = \left(\frac{-4}3\right)^k \Longrightarrow  \frac{8192}{1458} = \left(\frac{4}3\right)^k \Longrightarrow k =
...
\olatLinkGESOKompendium{3.3.1}{26}{29}


\newpage


\subsection{Transformationen\GESO{ (Streckung, Spiegelung)}}\index{Manipulation!Funktionen}\index{Funktions-Manipulation}\index{Transformation!Funktion}
...
\TALS{
$$f(x) = a\cdot{}\left(\frac{x-d}b\right)^4+c$$
und
$$g(x) = a\cdot{}\left(\frac{x-d}b\right)^5+c$$
}%% end TALS
...
und beschreiben Sie die Effekte \TALS{der Parameter} \GESO{des Parameters}

\TALS{Was sind die Standard (default)-Werte für $a$, $b$, $c$ und $d$?
  Diejenigen Werte, welche die Normalparabeln ($y=x^4$, $y=x^5$) ergeben?
  \TNT{2}{$a=1$, $b=1$, $c=0$ und $d=0$}}
...
$d$: \TRAINER{Verschiebung entlang der $x$-Achse (Verschiebung nach
  rechts verlangt ein positives $d$.)}
...
\subsubsection{Zusammenfassung\TALS{ der Transformationen}}
...
\TALS{
  \aufgabenFarbe{Zeichnen Sie mit \texttt{geogebra.org} die Funktion
    ...
    \\
    Definieren Sie anschließend die Funktion $$g(x) = a\cdot{}f\left(\frac{x-d}{b}\right)+c.$$
\\
    Entscheiden Sie, welche Veränderungen die Parameter $a$, $b$, $c$
    und $d$ bewirken.\\
    Starten Sie mit $a=1$, $b=1$, $c=0$ und $d=0$.
  }%% END Aufgabenfarbe
  \TNT{4}{
    $c$: Verschiebung nach oben\\
...
\AadBMTA{304ff}{9. a) d) e), 5. a) c) \TALS{d) }e)}
...
\olatLinkGESOKompendium{3.3.1.}{26}{28.}
\newpage
