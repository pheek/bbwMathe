%%
%% 2019 07 04 Ph. G. Freimann
%% 2020 Systematik verbessert
%%

\section{Lineare Funktionen}\index{Funktion!lineare}
\index{affin-lineare Funktionen}\label{lineare_funktionen}
\sectuntertitel{Eine Gerade, endlos klar, ohne Wendung, wunderbar.}
%\sectuntertitel{$f: \mathbb{R} \rightarrow \mathbb{R}$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Lernziele}

\begin{itemize}
\item Begriff «Lineare Funktion»
\item Darstellungen (Wertetabelle, Funktionsgleichung, Graph)
\item Charakteristische Punkte\index{Punkt!charakteristischer}
  \begin{itemize}
  \item
    y-Achsenabschnitt (Ordinatenabschnitt)
  \item Nullstelle
  \end{itemize}
\item Steigung
\TALS{\item Taschenrechner: Darstellung von Graphen}
\end{itemize}

%%Theorie:
%%\TALS{(\cite{frommenwiler17alg} S.170 (Kap. 3.3))}
%%\GESO{(\cite{marthaler21alg}       S.237 (Kap. 14))}


\GESO{\matheNinjaLink{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/108030887418864}}
\TALS{\matheNinjaLink{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/108030908044855}}

\TadBMTA{237}{14}

\newpage

\subsection{Einstiegsbeispiel Taxiunternehmen}
Taxiunternehmen «A» hat eine Grundgebühr von CHF 10.- und kostet
danach CHF 0.60 pro gefahrenen Kilometer.

Taxiunternehmen «B» hat eine Grundgebühr von CHF 6.- und kostet
danach CHF 0.90 pro gefahrenen Kilometer.

Ab bzw. bis wo lohnt sich das Unternehmen «A» bzw. «B»?

  Zeichnen Sie ein Koordinatensystem. $x$-Achse = km; $y$-Achse = CHF.
  (Einteilung je ca. 20 Einheiten im 1. Quadranten.

\TRAINER{%%
    \bbwCenterGraphic{14cm}{allg/funktionen/img/Taxiunternehmen.png}

  Ablesen bei ca. 13-14 km.

  Rechnerisch (A(x): Preis Unternehmen A in CHF für x km):

  $$A(x) = 10\text{[CHF]}
  +x\text{[km]}\cdot{} 0.60 \left[\frac{\text{CHF}}{\text{km}}\right] $$
  $$B(x) = 6 +x \cdot{} 0.90$$
  Gleichstand bei:
  $$A(x) = B(x)$$
  $$10+0.6x = 6+0.9x$$
  Lösung (TR):
  $$x = 13.\overline{3} \text{km}$$
}%% END TRAINER
\noTRAINER{%
    \bbwCenterGraphic{14cm}{allg/funktionen/img/LeerFuerTaxi.png}%
}%% end noTRAINER
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Definitions- und Wertemenge}

\begin{definition}{Definitionsmenge $\mathbb{D}$}{}
  
Mit \textbf{Definitionsmenge}\index{Definitionsmenge|textbf} $\mathbb{D}$ (auch
Definitionsbereich\index{Definitionsbereich|textbf}) wird die Menge aller
möglichen Funktionsargumente bezeichnet, kurz alle Zahlen, die für $x$
eingesetzt werden dürfen. 
\end{definition}

\begin{definition}{Wertemenge $\mathbb{W}$}{}
Unter der \textbf{Wertemenge}\index{Wertemenge|textbf} (auch Wertebereich\index{Wertebereich|textbf} oder Bildmenge\index{Bildmenge}) $\mathbb{W}$
verstehen wir die Menge aller getroffenen Werte aus der
Grundmenge.\TALS{\footnote{Bemerkung: im Buch \cite{marthaler21alg} wird
  mit dem Begriff «Bildmenge» der Wertevorrat bezeichnet!}}
\end{definition}

\begin{definition}{Grundmenge $\mathbb{G}$}{}
Die \textbf{Grundmenge}\index{Grundmenge} ($\mathbb{G}$) und der \textbf{Wertevorrat}
sind bei uns stets $\mathbb{R}$. Definitions- und Wertemenge sind also
Teilmengen von $\mathbb{R}$.
\end{definition}
\newpage

\begin{beispiel}{Definitions- und Wertemenge}{}
  In obigem Taxi-Beispiel $A(x) = 0.6\cdot{}x + 10$ sind:

  \vspace{3mm}
  $$\mathbb{D} = \LoesungsRaum{\mathbb{R^{+}}}$$


  \vspace{3mm}
  $$\mathbb{W} = \LoesungsRaum{[10; \infty{}[}$$

\end{beispiel}

\subsubsection*{Begriffe}

  \begin{bbwFillInTabular}{l|l}
    \textit{Begriff} & \textit{Beispiel(e)}\\\hline
    \textbf{Funktionsgleichung}\index{Funktion!Gleichung}\index{Gleichungen!Funktion}
    & \TRAINER{$y=0.6x+10$}\noTRAINER{\hspace{70mm}\,}\\\hline
    \textbf{Funktionsargumente}\index{Funktion!Argument}\index{Argument!Funktionen}
    & \TRAINER{$x$-Argumente:  1, 2, 5 (km)}\\\hline
    \textbf{Funktionswert}     \index{Funktion!Wert}\index{Wert!einer
      Funktion}                    & \TRAINER{$y$-Werte: 10.60, 11.20,
      13.- (CHF)}\\\hline
    \textbf{unabhängige Variable}\index{Funktion!unabhängige Variable}\index{unabhängige Variable} & \TRAINER{$x$}\\\hline
    \textbf{abhängige Variable}\index{Funktion!abhängige Variable}\index{abhängige Variable}       & \TRAINER{$y$}\\\hline
    Funktionsterm\index{Funktion!Term}\index{Funktionsterm}\index{Terme!Funktionsterm}
    & \TRAINER{$0.6x+10$}\\
  \end{bbwFillInTabular}


\newpage


\subsection{Taschenrechner}\index{Taschenrechner!Funktionen}

  Mit der «TABLE»-Funktion\TALS{\footnote{Im «Graphs» Modus mit
      CTRL-T (oder über Menu 7: Tabelle mit geteiltem Bildschirm)}} auf Ihrem Taschenrechner lassen sich Wertetabellen ganz einfach erstellen.
  Füllen Sie eine Wertetabelle zur Funktion $f: y=\frac{3}{4}x  - 1.5$ aus und zeichnen Sie die Gerade in ein rechtwinkliges Koordinatensystem:

  \begin{tabular}{l|c|c|c|c|c|c|c}
    $x$ & -3 & -1 & 0 & 1 & 2 & 4 \\
    \vphantom{b}&&&&&&\\
    $y$ & \LoesungsRaumKurz{-3.75}   & \LoesungsRaumKurz{-2.25}   & \LoesungsRaumKurz{-1.5}  & \LoesungsRaumKurz{-0.75}  & \LoesungsRaumKurz{0}  &  \LoesungsRaumKurz{1.5} \\
  \end{tabular}

  
  \bbwGraph{-4}{4}{-4}{2}{
    \bbwDot{-3,-3.75}{black}{north}{}
    \TRAINER{\bbwFunc{0.75*\x - 1.5}{-2.5:3.5}
       \bbwDot{-1,-2.25}{black}{north}{}
       \bbwDot{0,-1.5}{black}{north}{}
       \bbwDot{2,0}{black}{north}{}
       \bbwDot{4,1.5}{black}{north}{}
    }%% end Trainer
  }%% END bbw Graph


\newpage%%

\subsection{Grundform der linearen Funktion}\index{Lineare Funktion!Definition}

\begin{definition}{Lineare Funktion}{}
  Eine Funktion $f: x\mapsto f(x)$ von $\mathbb{R}$ nach $\mathbb{R}$ heißt
\textbf{linear}\footnote{Die linearen Funktionen werden eingeteilt in die \textbf{Proportionalitäten} für $b=0$ und in die \textbf{affinen Abbildungen} für $b\ne{}0$.}\index{Lineare Funktion}, wenn sie in der Grundform
$$f(x) = y=a\cdot{}x + b$$
mit $a$ und $b$ in $\mathbb{R}$ dargestellt werden kann.
\end{definition}


Beispiele:

\vspace{3mm}
\hrule
\vspace{3mm}

$$f(x) = y = \LoesungsRaumLang{4x + 1.5}$$
(Hier ist $a=\LoesungsRaum{4}$ und $b=\LoesungsRaum{1.5}$.)

\vspace{3mm}
\hrule
\vspace{3mm}

$$f(x) = y = \LoesungsRaumLang{-x}$$
(Hier ist $a=\LoesungsRaum{-1}$ und $b=\LoesungsRaum{0}$.)

Ist $b=0$, so sprechen wir auch von einer Proportionalität.

\vspace{3mm}
\hrule
\vspace{3mm}

$$f(x) = y = \LoesungsRaumLang{-0.6}$$
(Hier ist $a=\LoesungsRaum{0}$ und $b=\LoesungsRaum{-0.6}$.)

Ist $a=0$, so sprechen wir auch von der konstanten Funktion.

\vspace{3mm}
\hrule
\newpage

\subsection*{In Grundform bringen}

Bringen Sie die folgenden Funktionen in die Grundform (falls es sich
um lineare Funktionen handelt):

$$f(x)=\frac{x}{3} - \sqrt{2} + \pi\cdot{}x - \frac{1}{7}\cdot{}x + \frac{5}{3} -2.5x$$
\TNT{4}{
  sortieren:
  
  $f(x) = \frac{1}{3}\cdot{}x + \pi\cdot{}x - \frac{1}{7}x -2.5x +
  \frac{5}{3} - \sqrt{2}$

  $x$ ausklammern und wo möglich zusammenfassen

  $f(x) = \left(\frac{1}{3} - \frac{1}{7} + \pi - 2.5\right)\cdot{}x +
  \left(\frac{5}{3} - \sqrt{2}\right) = \left(\frac{-97}{42} +
  \pi\right)x + \left(\frac{5}{3} - \sqrt{2}\right) $
}

$y=x+2x-7+\frac{x}3$

\TNT{1.2}{ist linear: $y= \frac{10}{3}x-7$}

$f(x) = x+3x + x^2$

\TNT{1.2}{ist nicht linear!}

$2y+x = 4$

\TNT{1.2}{ist linear: $2y=4-x$ ergo $y = \frac{-1}{2}\cdot{}x + 4$}

$x\mapsto \frac{1}{3}x+\frac13 + \frac{x}{4} -\frac14$

\TNT{1.2}{ist linear $x\mapsto \frac{7}{12}x + \frac{1}{12}$}

$y = \frac{x}{3} + \frac{3}{x}$

\TNT{1.2}{ist nicht linear}

$g(x) = \frac{x}{a} + c + bx$

\TNT{1.2}{ist linear $g(x) = \left(\frac{1}{a} + b\right)\cdot{}x +
  c$}
\newpage


\subsection*{Aufgaben}
%%\GESOAadBMTA{250}{6. a) b)}

\GESO{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{1. Funktionen
    skizzieren (1.1 und einzelne aus 1.2) }}
\TALS{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{1. Funktionen
    skizzieren (1.1 und einzelne aus 1.2) }}


\newpage%%


\subsection{Charakteristiken}
%%
%%\subsection{Charakteristische Punkte}\index{Punkte!charakteristische}\index{charakteristische Punkte}
Eine typische lineare Funktion schneidet irgendwo die beiden Achsen\footnote{Ausnahme: Konstante. Eine Konstante $y=b$ schneidet nur die $y$-Achse. Der Spezialfall $y=0$ \textbf{ist} die $x$-Achse.}. Zeichnen Sie die Funktion $f: y=\frac{1}{2}x  +3$ und notieren Sie die Punkte, wo die $x$- bzw. die $y$-Achse von unserer Geraden geschnitten wird:

\vspace{1cm}

Wertetabelle:


\begin{tabular}{c|p{2cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}}
   $x$  & 0 & 1 & 2 & 4 & -4 & \TRAINER{-6}\\\hline
   $y$  & \TRAINER{3} & \TRAINER{3.5} & \TRAINER{4}& \TRAINER{5}&\TRAINER{1}&0\\%%
\end{tabular}


\TNT{4}{
  Die Nullstelle bei $y=0$ wird entweder abgelesen, oder berechnet,
  indem wir in der Funktionsgleichung $y=\frac12x+3$ den Funktionswert
  $y=0$ setzen:
  $$0=\frac12x+3 \Longrightarrow x_0=-6$$
}

\bbwGraph{-7}{7}{-2}{5}{\TRAINER{
    \bbwDot{-6,0}{red}{east}{N}
    \bbwDot{0,3}{orange}{west}{Y}
    \bbwFuncC{3+0.5*\x}{-6.5:3.5}{blue}
}}
\newpage

\subsubsection{$y$-Achsenabschnitt}\index{$y$-Achsenabschnitt}\index{Achsenabschnitt!y-}\index{Ordinatenabschnitt}
\begin{definition}{$y$-Achsenabschnitt}{}
  Der Parameter $b$ wird $y$-Achsenabschnitt
(oder Ordinatenabschnitt\index{Ordinatenabschnitt}\footnote{Die
    $y$-Koordinate eines Punktes wird auch
    \textbf{Ordinate}\index{Ordinate!lineare Funktionen} und die $x$-Koordinate wird auch
    \textbf{Abszisse}\index{Abszisse}  genannt.}) genannt.
  
Dieser gibt an, in welcher «Höhe» die Gerade die $y$-Achse schneidet.
\end{definition}

Wir erhalten den $y$-Achsenabschnitt, indem wir das Funktionsargument $x$
gleich Null (0) setzen: $x=0$.

Somit wird
$$y=ax+b$$
$$y=a\cdot{}0+b$$
$$y=b$$
Was uns zeigt, dass der  Punkt $P=(0|b)$ der Geraden immer auf der $y$-Achse liegt.

\begin{beispiel}{$y$-Achsenabschnitt}{}
  Was ist der y-Achsenabschnitt der Funktion $y=\frac78 x- 13.45$

  \TNT{2}{$y$-Achsenabschnitt ist hier $-13.45$}
\end{beispiel}


\subsection*{Aufgaben}
%%\GESOAadBMTA{250}{6. a) b)}

\GESO{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{2. $y$-Achsenabschnitt}}
\TALS{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{2. $y$-Achsenabschnitt}}




\newpage


\subsubsection{Steigung}\index{Steigung|textbf}
  \bbwCenterGraphic{6cm}{allg/funktionen/img/starkeSteigung2.jpg}
  \begin{center}\tiny{Vietnam 2025}\end{center}
  
Der Parameter $a$ wird Steigung der Geraden genannt. Dies entspricht
dem $y$-Anstieg\footnote{Bei negativem $a$ sprechen wir von \textbf{Gefälle}\index{Gefälle},
  Abstieg, Neigung, Senkung, ...} der Geraden pro Einheit in
$x$-Richtung ($\e_x = 1$):


%% Steigungsgraph: Eins nach rechts -> a noch oben (oder -a nach unten)
\bbwGraph{-4}{4}{-3}{4}{
	\draw[thick] (-4,-2.5) -- (4, 3.5);
	\draw[thick,color=blue] (1, 1.25) -- (2  , 1.25);
	\draw[thick,color=blue] (1, 1.25) -- (1.5, 1.25) node[anchor=north]{${\color{blue}1}$};
	\draw[thick,color=red ] (2, 1.25) -- (2, 2   );
	\draw[thick,color=red ] (2, 1.25) -- (2, 1.625   ) node[anchor=west ]{${\color{red}a}$ };
}

\newpage
\subsection*{Steigungsdreieck}\index{Steigungsdreieck}
Die Steigung $a$ kann aus \textbf{jedem} Steigungsdreieck berechnet werden,
wozu einfach zwei Punkte auf der Geraden zu verwenden sind:

%% Steigungsgraph: Eins nach rechts -> a noch oben (oder -a nach unten)
\bbwGraph{-4}{4}{-3}{4}{
	\draw[thick] (-4,-11/5) -- (4, 13/5);


  \TRAINER{
    \draw[thick,color=blue] (-2, -1) -- (3,-1);
	  \draw[thick,color=blue] (1, -1.1) node[anchor=north]{${\color{blue}H}$};
    
	  \draw[thick,color=red] (3, -1) -- (3,2);
    \draw[thick,color=red] (3, 1.1) node[anchor=west]{${\color{red}V}$};
  }

  \TRAINER{
    \draw[thick,color=blue] (-3.7, -2  ) -- (-2.7,-2);
	  \draw[thick,color=blue] (-3.2, -2.1) node[anchor=north]{${\color{blue}1}$};
    
	  \draw[thick,color=red] (-2.7, -2  ) -- (-2.7,-1.4);
    \draw[thick,color=red] (-2.7, -1.6) node[anchor=west]{${\color{red}a}$};
  }

%%  \TRAINER{
%    \draw[color=blue] (-3.9, 3) node[anchor=west]{${\color{red}V}:{\color{blue}H} = {\color{red}a} : {\color{blue}1}$};
%%    \draw[color=blue] (-3.9, 2)
%%    node[anchor=west]{$\frac{{\color{red}V}}{{\color{blue}H}} =  {\color{red}a}$};

%%    \draw (-3.9, 1) node[anchor=west]{Hier $a=\frac34$};
  %%r}

  \bbwDot{-2,-1}{black}{north}{}
  \bbwDot{3,2}{black}{north}{}
  \bbwDot{3,-1}{black}{north}{}
}

\TNTeop{
  Das Steigungsdreieck $V:H$ ist ähnlich zum Dreieck $a:1$:

  ${\color{red}V}:{\color{blue}H} = {\color{red}a} : {\color{blue}1}$

  Und somit:
  
  $\frac{{\color{red}V}}{{\color{blue}H}} =  {\color{red}a}$

Hier $a=\frac34$
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\subsection*{Gefälle}

Steigt die Gerade in $x$-Richtung an, so ist ${\color{ForestGreen}a}$ positiv
(${\color{ForestGreen}a>0}$) {\color{ForestGreen} «Steigung»};\index{Steigung}
fällt die Gerade in $x$-Richtung ab, so ist ${\color{orange}a}$ negativ
(${\color{orange}a<0}$) {\color{orange} «Gefälle»}.\index{Gefälle}

%% Steigungsgraph: Eins nach rechts -> a noch oben (oder -a nach unten)
\bbwGraph{-4}{4}{-3}{4}{
	\draw[thick,color=ForestGreen] (-4, -1) -- (4  , 3);
	\draw[thick,color=ForestGreen] (4, 3) node[anchor=north]{${\color{ForestGreen}+}$};
	\draw[thick,color=orange     ] (-3.5, 2) -> (4, -2);
	\draw[thick,color=orange     ] (4, -2) node[anchor=west ]{${\color{orange}-}$ };
}
\newpage

\begin{rezept}{Finden der Geradensteigung}{}
Die Steigung $a$ ist:
$$a=\frac{{\color{red}V}}{{\color{blue}H}}$$
«Vau durch Haa, das gibt das Ahh!»
\end{rezept}

\subsubsection*{Herleitung (optional)}

\bbwCenterGraphic{10cm}{allg/funktionen/img/Steigungsdreieck.png}

\TNTeop{

 $$\frac{V}H = \frac{\Delta y}{\Delta x} = \frac{y_2-y_1}{x_2-x_1} =
  \frac{f(x_1) - f(x_2)}{x_2-x_1}$$

  $$= \frac{(a\cdot{}x_2 + b) - (a\cdot{}x_1 +b)}{(x_2-x_1)}$$
  $$= \frac{a\cdot{}x_2 + b - a\cdot{}x_1 -b}{(x_2-x_1)}$$
  $$= \frac{a\cdot{}x_2 - a\cdot{}x_1 }{(x_2-x_1)}$$
  $$= \frac{a\cdot{}(x_2 - x_1 )}{(x_2-x_1)}$$
  $$= a$$
}%% END TNT eop

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage



\textbf{Ablesen}

\bbwCenterGraphic{180mm}{allg/funktionen/img/LineareFunktionenAblesen.png}

\TNTeop{
  rot       : $f_1(x) =  \frac{ 2}{3}x + 2   $\\
  braun     : $f_2(x) =  \frac{ 6}{7}x - 2   $\\
  dunkelgrün: $f_3(x) =  \frac{-2}{5}x + 3   $\\
  ultramarin: $f_4(x) =  \frac{-1}{3}x + 5   $\\
  fuchsia   : $f_5(x) =  \frac{-3}{5}x       $\\
  hellgrün  : $f_6(x) =               -  3   $\\
  lila      : $f_7(x) = -3           x-  4   $\\
  orange    : $f_8(x) = \frac{1}{4}x -\frac12$
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555

\subsection*{Aufgaben}
%%\GESOAadBMTA{250}{6. a) b)}


\GESO{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{3. Steigung}}
\TALS{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{3. Steigung}}
\newpage

\subsubsection{Nullstelle}\index{Nullstelle}
Eine steigende oder fallende lineare Funktion schneidet die $x$-Achse
in einem Punkt; dessen $x$-Koordinate wird \textbf{Nullstelle} genannt.

In der anfänglich untersuchten Funktion $f(x) = \frac12x+3$ schneidet
der Graph die $x$-Achse bei $-6$. 

\begin{definition}{Nullstelle}{}
  Eine \textbf{Nullstelle} $x_0$ zeigt an, an welcher Stelle der
  Funktionsgraph mit der $x$-Achse übereinstimmt.
\end{definition}


Die Nullstelle einer linearen Funktion in ihrer Grundform ist die Lösung der entsprechenden linearen Gleichung (mit
$y$=0) in ihrer Grundform. Nämlich:

\hrule

a) Lineare \textbf{Gleichung} $$ax+b=0$$

Gesucht \textbf{Lösung} $x$:


\TNT{6}{
  $$ax+b=0 | -b$$
  $$ax=-b | :a$$
  
}%% end TNT


\vspace{3mm}
$$\lx = \TRAINER{\left\{\frac{-b}{a}\right\}}  $$

\newpage
b) Lineare \textbf{Funktion} $$f: y=ax+b$$

Gesucht \textbf{Nullstelle} $x_0$ bei $y_0=0$. Die Gerade schneidet
die $x$-Achse in der Nullstelle $x_0$ der Geraden.

\begin{gesetz}{Nullstelle}{}
  Die Nullstelle $x_0$ einer linearen Funktion $$y=ax+b$$ ist die Lösung der
  entsprechenden linearen Gleichung in Grundform $$0=ax_0+b$$
  $$x_0 = \frac{-b}{a}$$
\end{gesetz}

\begin{beispiel}{Nullstelle bestimmen}{}
  Wo liegt die Nullstelle der folgenden Funktion?
  $$y=7x-3$$
  \TNT{6}{ Setze für $y$ Null ein:
    $$0 = 7x - 3$$
    $$3 = 7x$$
    $$\frac37 = x$$
    $$x_0 = \frac37$$
  }
  
\end{beispiel}
%Somit ist der
%Schnittpunkt (für $a\ne 0$) wie folgt gegeben:
%$$N = (x_0|y_0) = (x_0|0) =\left( \frac{-b}{a}\,\, \bigg|\,\, 0 \right) $$
\newpage

\subsection*{Aufgaben}

\GESO{\olatLinkArbeitsblatt{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{4.1 und 4.2}}
\TALS{\olatLinkArbeitsblatt{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{4.1 und 4.2}}


\GESO{\olatLinkArbeitsblatt{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{4.3 - 4.8}}
\TALS{\olatLinkArbeitsblatt{Lineare Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{4.3 - 4.8}}
%%\AadBMTA{251ff}{10., 23. a) c) und optional 20. (Alkohol im Blut)}
\newpage

\subsection{Referenzaufgaben}\index{Lineare Funktion!Referenzaufgaben}\index{Referenzaufgaben zu linearen Funktionen}

\input{allg/funktionen/ReferenzaufgabeSchnittpunkt}

\input{allg/funktionen/ReferenzaufgabeParalleleSenkrechte}

\input{allg/funktionen/ReferenzaufgabeEinPunktGegeben}

\input{allg/funktionen/ReferenzaufgabeGeradeDurchZweiPunkte}

\TALS{\input{tals/fct1/ReferenzaufgabeAbstand1}}

\GESO{
\subsection*{Vermischte Aufgaben zu linearen Funktionen}
\olatLinkGESOKompendium{3.2}{20ff}{3. bis 26.}
\newpage
}%% END GESO

\subsection{Lineare Gleichungen visualisieren\GESO{ (optional)}}\index{Gleichungen!visualisieren}\index{Visualisierung von Gleichungen}
Wir gehen von der linearen Gleichung $$(I)\hspace{5cm}5x-3.7 = 3x-0.7$$ aus.

Bringen wir alle Summanden nach links, so erhalten wir folgende
Gleichung:

\TNT{2}{$$5x-3x-3.7+0.7= 0$$
$$2x-3 = 0\hspace{5cm} (II)$$}%% END TNT


Schreiben wir dies als Funktion in $x$, so können wir folgende
Zuorndung definieren:

$$f(x): x \mapsto y = \TRAINER{2x-3}$$

Stellen wir diese Funktion nun graphisch dar, so ergibt sich folgendes
Bild:

%\TRAINER{\raisebox{-1cm}{\includegraphics[width=7cm]{allg/funktionen/img/FunktionsplotAusGleichung.png}}}
%\noTRAINER{\raisebox{-1cm}{\includegraphics[width=7cm]{allg/funktionen/img/KoordSystemFaktor20.png}}}


  \bbwGraph{-1}{2}{-4}{2}{
%%    \bbwDot{-3,-3.75}{black}{north}{}
    \TRAINER{
       \bbwFunc{2*\x - 3}{-0.5: 2}
%       \bbwDot{-1,-2.25}{black}{north}{}
%       \bbwDot{0,-1.5}{black}{north}{}
%       \bbwDot{2,0}{black}{north}{}
       \bbwDot{1.5,0}{black}{north}{}
       \bbwDot{0,-3}{black}{north}{}
    }%% end Trainer
  }%% END bbw Graph


Nullstelle $x_0 = 1.5$. Das heißt, $x_0 = 1.5$ löst Gleichung $(II)$
und somit auch Gleichung $(I)$ (die beiden Gleichungen sind äquivalent).


%%Die Lösung obiger Gleichung kann nun auf der $x$-Achse ungefähr bei $x=1.0$ abgelesen werden.
\newpage

\subsection*{Aufgaben}
%%\TALSAadBFWA{171ff}{601. a) b) c) e) f) 602. b) c) d) 603. b) c)
%% 604. 605. a) c) e) 606. a) c) 607. c) d) g) 608. a) 609. a) 610. a)
%% 611. a) 613. a) 616. c) 622. Flächen: 626. 628. und 629.}

\DeclareRobustCommand{\tempA}{9.1 und 9.2}
\GESO{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/102901171745363}{\tempA}}
\TALS{\olatLinkArbeitsblatt{Lineare
    Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029786/CourseNode/106131926046623}{\tempA}}

\GESO{\olatLinkGESOKompendium{3.2}{25}{27.}}

\GESO{
  Alte Maturprüfungsaufgaben zu linearen Funktionen:
  
  \olatLinkArbeitsblatt{Alte Maturprüfungsaufgaben zu linearen Funktionen}{https://olat.bms-w.ch/auth/RepositoryEntry/6029794/CourseNode/107461640594784}{05\_Lineare\_Funktionen\_alte\_Prüfungsaufgaben\_GESO.pdf}
  %%\newpage \input{allg/funktionen/AusAltenMaturapruefungenGESO}
}%% end GESO

\newpage%%
