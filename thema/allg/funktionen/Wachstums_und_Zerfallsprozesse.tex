...
Zu Punkt 5.\GESO{ (optional)}: $f(t) = b\cdot{}\e^{m\cdot{}t}$ (Basis $\e$):
\TNT{6.8}{
  Auch hier:
...
Ansatz:
\begin{center}\fbox{$b\cdot{}a^{\frac{t}{\tau}} =
    b\cdot{}a_2^{\frac{t}{\tau_2}} \TALS{ =    b\cdot{}\e^{m\cdot{}t}}  $}
\end{center}

(Teilen durch $b$ und Ziehen der $t$-ten Wurzel:)

\begin{gesetz}{Basiswechsel}{}

  \begin{center}\fbox{\fbox{$a^{\frac{1}{\tau}} =
        a_2^{\frac{1}{\tau_2}} \TALS{ = \e^{m}}$}}\end{center}
...
\TALS{  \vspace{3mm}
  $m=\ln\left(a^\frac1\tau\right) =
  \ln\left(a_2^\frac1{\tau_2}\right)$\\
  }%% end TALS
\end{gesetz}
\newpage
...
Die Halbwertszeit wird insbesondere
  beim radioaktiven Zerfall verwendet.\footnote{Beispiel: Nach wie viel tausend Jahren strahlt
