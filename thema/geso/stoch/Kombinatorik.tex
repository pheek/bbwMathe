...
sind denkbar? Wie sähe es aus, wenn die Buchstaben wiederholt werden könnten (BBC, ABB)?}%% ENd Aufgabe
\TNT{2.4}{«Ohne Zurücklegen»: $\frac{7!}{(7-3)!} = 210$. «Mit Zurücklegen»: $7^3=343$}%% END TNT
...
\subsubsection{Mississippi-Formel (optional)}\index{Mississippi-Formel}\index{Multinomialkoeffizient}
...
\aufgabenFarbe{Swiss LOTTO (ohne den Stern): Auf wie viele Arten
  ...
  \TNTeop{${42\choose 6}$ \TRAINER{$ = 5\,245\,786$}}%% END TNT
  \newpage
  
\aufgabenFarbe{Mittagessen: Ein Koch kann auf dem Markt aus vier
  Gemüsesorten, sechs Arten von Pilzen, drei
  Salatsorten und aus acht Arten von Früchten auswählen.
\\
  Er will ein Menü aus zwei Gemüsesorten, drei Pilzarten und einem
  Salat herstellen. Zum Dessert wählt er drei Arten von
  Früchten aus.
\\
  Auf wie viele Arten kann sich der Koch entscheiden?}
  \TNT{2.4}{${4 \choose 2} \cdot{} {6 \choose 3} \cdot{} {3 \choose 1}
    \cdot{} {8 \choose 3} = 20\,160$}%% END TNT
...
\TNT{3.2}{$$N= { {3+10-1} \choose {10} } = {12 \choose 10}  = 66$$}
\end{beispiel}
