...
Farben darf jedoch mehrfach vorkommen, die Reihenfolge sei hier
wesentlich.
...
Dieses Experiment entspricht im \textbf{Urnenmodell}\index{Urnenmodell} der...
...
\noTRAINER{\vspace{16mm}}
    
  $k$ &=& \TRAINER{\textbf{Vorgeschriebene Anzahl an Ziehungen}}\noTRAINER{\vspace{10mm}}\\
...
Dies entspricht im Urnenmodell der...
...
\TRAINER{$25-5:5 = 25-(5:5) = 25 - 1 = 24 = 4\cdot{}3\cdot{}2\cdot{}1
  = 4!$, aber $(25-5):4 = 4 (!)$}
...
der Taste \tiprobutton{math}\tiprobutton{5}.
\end{bemerkung}
...
Nun haben wir zehn solcher Bücher zur Auswahl. Auf wie viele Arten können wir nun sechs Bücher in unser Regal einreihen?
...
\begin{bemerkung}{}{}
Wenn wir \textbf{alle} $n$ Objekte geordnet auswählen, so erhalten wir
...
\subsubsection{Freie Plätze? (optional)}
...
& \raisebox{20mm}{\parbox{40mm}{Wieder gilt:$$n=\LoesungsRaumLen{27mm}{5\, \text{optional}}$$ $$k=\LoesungsRaumLen{27mm}{3\,\text{zwingend}}$$}} & \includegraphics[width=5cm]{geso/stoch/img/dreiPersonenAufFuenfStuehle.png}\\\hline
...
\aufgabenFarbe{Auf wie viele Arten können sich Hanna, Igor, Jana, Karl
und Lena auf die acht Plätze ganz hinten im Bus platzieren?}
\TNT{3.2}{$$N=\frac{8!}{(8-5)!}  = \frac{8!}{3!} = 6720$$}
\newpage
...
Der sog. \textbf{Multinomialkoeffizient} ist eine spezielle Form der Fakultät.
...
\begin{gesetz}{Multinomialkoeffizient}{}
  ...
  $N$ ist die Anzahl Permutationen unter Berücksichtigung, dass jeweils $k_i$ Elemente nicht unterscheidbar sind.
\end{gesetz}
...
Unsinnsworte sind mit den elf Buchstaben des Wortes «Mississippi»
möglich?
...
\AddToShipoutPictureBG{
    \AtPageLowerLeft{
        \put(\LenToUnit{132mm},\LenToUnit{211mm}){
            \includegraphics[
%                width=\LenToUnit{60mm}, %% dies war der KI vorschlag
%                \LenToUnit. doch mit exakter länge geht es wohl besser???:
%                height=\LenToUnit{40mm},
                width=60mm,
                height=40mm,
           %     keepaspectratio=false
            ]{geso/stoch/img/Familie_G_aus_W.png}%
        }%
    }%
}%
...
\begin{bemerkung}{Erweiterung der \texttt{nPr}-Formel (optional)}{}
  ...
  $${n \choose k} = \frac{n!}{k!(n-k)!} = \frac{n \text{ «nPr» } k}{k!} =
  n \text{ «nCr»} k$$
  \end{bemerkung}
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Beim Glücksspiel «Euromillions» werden fünf aus 50 Zahlen angekreuzt. Auf wie viel Arten ist dies möglich?
...
Die wohl ursprüngliche Namensgebung (optional):

\begin{bemerkung}{Binomialkoeffizient (optional)}{}
...
\subsubsection{Kombinationen mit Wiederholung\TRAINER{ (optional)}}\index{Kombination!mit Wiederholung}\label{kombiVariationMitWiederholung}
...
Ein Bäcker bietet \textit{Gipfeli} in drei Sorten an. Ich muss zehn
\textit{Gipfeli} kaufen. Wie viele Kombinationen gibt es?

\TNT{8}{$n=3, k=10$

  $$N= { {3+10-1} \choose {10} } = {12 \choose 10}  = 66$$}
\end{beispiel}

\TNTeop{
Begründung, falls verlangt:
  
  1 = Gipfeli Sorte 1 (Bsp. Butter)

  2 = Gipfeli Sorte 2 (Bsp. Laugen)

  3 = Gipfeli Sorte 3 (Bsp. Vollkorn)
  ...
  1. Sorte | 2. Sorte | 3. Sorte

  Beispiele
  
  xxxxxxxxxx | | «10 der 1. Sorte»

  xxxxxx | | xxxx   «6 der 1. Sorte 4 der 3. Sorte»

  xx |xxx| xxxxx  «2 der 1. Sorte, 3 der 2. Sotre, 5 der 3. Sorte

  Total $n$ Kreuze + $k-1$ Balken

  Kombinationen wie Mississippi - Formel
...
\subsection*{Aufgabe}
\aufgabenFarbe{
  Süßgetränke werden in 10 Sorten Angeboten. Ich kaufe für einen
  Grillabend 3 Getränkeflaschen. Wie viele Kombinationen sind möglich,
  wenn die Reihenfolge keine Rolle spielt und jede Getränkesorte auch
  mehrfach ausgewählt werden darf?
}%%
\TNT{2}{${3+10-1 \choose 3} = 220$}

\newpage
\input{geso/stoch/KombinatorikZusammenfassung}\newpage%


\newpage

\subsection{Vermischte Aufgaben zur Kombinatorik}
...
\\
  «ab», «an», «aus», «be», «fort», «nach», «über», «unter», «ver», «vor» und «zu»
\\
  in einem Gedicht verwenden und diese mit den Verben
\\
«fahren», «gehen», «geben», «kommen», «laufen», «lassen», «schlagen»,\\
«schreiben», «setzen», «tragen», «werfen» und «ziehen»
\\
kombinieren. Wie viele solche Kombinationen aus Vorsilbe und Verb (wie
«fortlassen» oder «angeben») sind damit möglich? Ergeben alle diese
Wörter einen Sinn? Sind alle davon Verben? Finden Sie einige unsinnige
Wörter, oder Kombinationen, die keine Verben ergeben.}
\TNTeop{
Theoretisch gibt es $11\cdot{}12 = 132$ Möglichkeiten. Welche Bedeutung
dem Wort «fortschlagen», «überkommen», «unterfahren», «untertragen» zukommt sei hier jedoch zweitrangig. Ebenso
sind nicht alles unbedingt nur Verben: «ich bin verschlagen», «das Abkommen», «der Antrag,
aber «beantragen, herantragen», «untergeben», «untersetzt»}%% ent TNTeop

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555

\aufgabenFarbe{Ein Monitor-Pixel (\textit{picture element}) hat die
  Möglichkeit in den Grundfarben \textbf{rot}, \textbf{grün} und
  \textbf{blau} (RGB) zu leuchten. Jede dieser Grundfarben kann dabei $2^8$
  Helligkeitsstufen verwenden. Wie viele Farben lassen sich damit mischen?}
\TNT{4}{$$N = \left(2^8\right)^3 = 16\,777\,216$$}
%\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5555



\aufgabenFarbe{Aus einem Team von 13 Fußballerinnen soll eine
  Delegation von drei Mitgliedern und aus dieser Delegation eine
  Sprecherin bestimmt werden. Auf wie viele Arten ist dies möglich?}
\TNT{2.4}{${13 \choose 3} \cdot{} {3 \choose 1} = 858$}
...
\aufgabenFarbe{
Vier Personen steigen in einen Lift im Parterre. Der Lift fährt mit diesen vier
Personen nach oben los und kann dabei in 6 darüber liegenden
Stockwerken anhalten.
\\
a) Auf wie viele Variationen können die vier Personen die Stockwerke
wählen, wenn auch mehrere Personen im selben Stockwerk (gleichzeitig)
den Lift verlassen können?
\\
b) Wie viele Variationen gibt es, wenn jeder Fahrgast ein anderes
Stockwerk auswählt?}
\TNT{6}{a) $$N = 6^4 = 1296$$
b) $$N = \frac{6!}{(6-4)!} = 360$$}

\aufgabenFarbe{(optional; schwierig) Das obige Zahlenschloss (vier
  Ringe, zehn Ziffern) wird so verdreht, dass eine
Ziffer maximal zweimal auftreten kann. Wie viele Varianten gibt es nun?}
\TNTeop{Es sind 9630 Varianten. Dies geht am einfachsten mit
...
$10000-10-360=9630$.
\\
2. Rechenweg: Muster «XYZT» + «XYZZ»(6x) + «XXYY»(3x)
und alle Vertauschungen: $10\cdot{}9\cdot{}8\cdot{}7 +
6\cdot{}(10\cdot{}9\cdot{}8) + 3\cdot{}(10\cdot{}9) = 9630$
}%% implicit \newpage



\aufgabenFarbe{An einer Party mit vierundzwanzig Gästen stößt jeder
  Gast mit jedem an. Wie oft erklingen die Gläser?}
\TNT{2.4}{${24 \choose 2} = 276$}


\aufgabenFarbe{Wie viele Personen nehmen an einer Party teil, wenn im
  ganzen 595 Mal die Gläser erklingen (und dabei jede Person mit jeder andern angestoßen hat)?}
\TNTeop{${n \choose 2} = \frac{n \cdot{} (n-1)}2 = 595
  \Longleftrightarrow \frac12n^2 - \frac12n -595 = 0
  \Longrightarrow a=0.5, b=-0.5, c=-595 $. Danach die
  Mitternachtsformel: $n=35$, die negative Lösung ist hier nicht von Belang.}
\newpage


\aufgabenFarbe{Smarties gibt es in 8 Farbsorten. Ich entnehme einer
  großen Packung 16 Smarties für meine Schülerinnen bzw. Schüler. Wie viele Farbkombinationen sind
  möglich, wenn die Reihenfolge keine Rolle spielt?
}
\TNT{2}{${8+16-1 \choose 16} = 245\,157$}
...
sind denkbar? Wie sähe es aus, wenn die Buchstaben wiederholt werden
könnten (BBC, ABB)?}
%% End Aufgabe
\TNTeop{«Ohne Zurücklegen»: $\frac{7!}{(7-3)!} = 210$. «Mit
  Zurücklegen»: $7^3=343$}%% implicit \newpage
...
\aufgabenFarbe{Swiss LOTTO (ohne den Stern): Auf wie viele Arten
  ...
  \TNT{4}{${42\choose 6}$ \TRAINER{$ = 5\,245\,786$}}%% END TNT
