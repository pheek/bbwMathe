...
$$P(E=17) = {20 \choose 17}\cdot 0.85^{17}\cdot (1-0.85)^{20-17} =  {20 \choose 17}\cdot 0.85^{17}\cdot (0.15)^{3}\approx{}24.28\%$$
...
\aufgabenfarbe{Kompendium: Kap. 5.7. Aufg. 36. a) b) und 37.}
\newpage

\subsection{Wahrscheinlichkeitsverteilungen ...}\index{Wahrscheinlichkeitsverteilung}

Für ein Bernoulli-Experiment können wir uns auch ein Histogramm\index{Histogramm} aufzechnen.
...
Zeichenen Sie in folgender Aufgabe beide Diagarmme: Die
...
\aufgabenfarbe{Kompendium: Kap. 5.7. Aufg. 36. c)}
...
Andererseits gibt es dazu auch eine Formel, die \textbf{hypergeometrische Verteilung}\footnote{Die Folgeglieder der \textit{geometrischen Verteilung} $P(X=t) = p\cdot{}q^{t-1}$ verhalten sich wie eine geometrische Reihe (die Quotienten von zwei Folgegliedern $P(X=t) : P(X=t+1)$ sind jeweils konstant). Da die hier betrachtete Verteilung noch rascher ``abfällt'', wie die \textit{gemoetrische Verteilung}, so sprechen wir hier von \textbf{hyper}geometrisch.}.
...
\textbf{Beispiel 2}: Ich will bei zwei gezogenen Kuglen genau einen Treffer:
