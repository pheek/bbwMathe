...
\renewcommand{\arbeitsblattTitel}{Lineare Funktionen (V 3.3)}
...
$y=6x + 3$ & \LoesungsRaum{3} & \LoesungsRaum{$ -0.5 $} & \LoesungsRaum{6} \\\hline
...
\TNTeop{
a) $f(t) = 30 - 4.3\cdot{}\frac{t}{25} = \frac{- 4.3}{25}\cdot{}t + 30
= -0.172x + 30$

b) Nullstelle: $t_0 = \frac{-b}a = -b : a = 30 : \frac{4.3}{25} = 30 \cdot{} 25 : 4.3 \approx 174.4 \text{
...
$$y=2\cdot{}\frac53 -3 = \frac{10}3 - 3 = \frac13$$
}
\newpage
...
a) Zeichnen Sie das Ende des Stalagmiten ab seiner Entstehung (0 Jahre) als
...
\TRAINER{\bbwCenterGraphic{16cm}{img/Stalak.png}}
\newpage
...
\TNT{6}{%
 ...
 $$g(x) = 230 - \frac{x}{60}  [\text{ cm}]$$
}%% end TNT

d) Wie alt ist dieses Stalaktiten-Stalaktiten Paar, wenn wir heute den Stalagmiten als 34 cm über Boden messen?
\TNT{4}{$y= \frac{x}{180} = 34$ ergo $x=6\,120$ Jahre.}%% end TNT

e) Wo schneiden sich die Geraden $f$ und $g$? Mit anderen Worten: Wann
(nach deren Entstehung) wachsen die beiden zusammen?
\TNTeop{%
$$\frac{x}{180} = 230 - \frac{x}{60}$$
$$ x  = 41\,400 - 3x                $$
$$4x  = 41\,400                     $$
$$ x  = 10\,350 \text{[Jahre]}      $$
}%% end TNTeop
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Parallele und Senkrechte}
\subsection{Zeichnen}
...
\subsection{Parallel gesucht}
Gegeben ist die Gerade $y=4.8x - 2.3$. Gesucht ist eine Parallele
dazu mit $y$-Achsenabschnitt $\frac73$.

\TNTeop{
Da die Gerade parallel sein muss, hat auch sie die Steigung $4.8$. Der
$y$-Achsenabschnitt ist ja auch schon mit $\frac73$ gegeben. Folglich
lautet die neue Geradengleichung
$$y=4.8x + \frac{7}{3}.$$
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Senkrechte gesucht}
Geben Sie senkrechte zu den folgenden Geraden an:

a) $y=2x-3$

b) $y= \frac{-5}{13}\cdot{}x + 22.5$
...
\TNTeop{
a) $y= \frac{-1}{2}\cdot{} x + b$

b) $y= \frac{13}{5}\cdot{} x + b$

c) $y = \frac{-10}{17}\cdot{}x + b$, denn $1.7 = \frac{17}{10}$

d) Es gibt keine Funktion, die senkrecht auf der Horizontalen $y=-4$ steht.
}


\subsection{Senklechte mit $y$-Achsenabschnitt}
Gegeben ist die Gerade $f$ mit $f(x) = 1.2\cdot{}x + 4.36$

Finden Sie eine zu $f$ senkrecht stehende Gerade $g$, welche die
$y$-Achse bei $-4.8$ schneidet.

\TNTeop{
$$f(x) = \frac{6}{5} x - 4.36$$
Senkrecht stehend heißt die Steigung ist die Gegenzahl des Kehrwertes,
also $a_g = \frac{-5}{6}$.

Da der $y$-Achsenabschnitt von $g$ mit $-4.8$ gegeben ist lautet die
senkrecht-stehende Gerade einfach:
...
\noTRAINER{\mmPapier{4}}\TRAINER{$y=\frac13 x + 4$}
...
\subsection{Senkrechte durch gegebene Punkte}

Gegeben ist der Punkt $P=(5|3)$ und die Gerade $f$. Gesucht ist eine
Gerade $g$, die senkrecht zu $f$ steht und durch den Punkt $P$ geht.

a) $f(x) = 4x - 2$

b) $f(x) = -x + \pi$

c) $f(x) = \sqrt{2}\cdot{}x + 3.77$

\TNTeop{
a) Ansatz $g(x) = \frac{-1}{4}x + b$. Durch $P$ (einsetzen) liefert:
$$3 = \frac{-1}{4}\cdot{}5 + b$$
Ergo $b = 3 + \frac{5}{4} = \frac{17}{4}$. Die Funktionsgleichung
lautet:

$$g(x) = \frac{-1}{4}\cdot{}x + \frac{17}{4}$$

b) 
Ansatz $g(x) = 1\cdot{} x + b$. Durch $P$ (einsetzen) liefert:
$$3 = 5 + b$$
Ergo $b = 3 - 5 = -2$. Die Funktionsgleichung
lautet:

$$g(x) = x-2$$


c) Ansatz $g(x) = \frac{-1}{\sqrt{2}} x + b$. Durch $P$ (einsetzen) liefert:
$$3 = \frac{-1}{\sqrt{2}}\cdot{}5 + b$$
Ergo $b = 3 +\frac{5}{\sqrt{2}}$. Die Funktionsgleichung
lautet:

$$g(x) = \frac{-1}{\sqrt{2}}\cdot{} x  + 3 + \frac{5}{\sqrt{2}}
= \frac{-\sqrt2}{2}\cdot{}x + 3 + \frac{5\sqrt{2}}{2}$$

  
}

\subsection{Nullstelle: Parallele und Senkrechte}
Gegeben ist die lineare Funktion $x \mapsto 2x-4$. Zwei weitere
Funktionen $f$ bzw. $g$ sind dazu parallel bzw. stehen dazu senkrecht.
$f$ und $g$ haben die Nullstellen $x_0=5$.

Bestimmen Sie die Funktionsgleichung von $f$ bzw. von $g$.

\TNTeop{
Nullstelle bei $x_0 = 5$ heißt, dass die neuen Funktionen durch den
Punkt $N=(x_0 | ) = (5 | 0)$ verlaufen.

$f$ ist parallel, somit ist der Ansatz $f(x) = 2x + b$. $g$ steht
senkrecht und somit ist für $g$ der Ansatz
$g(x) = \frac{-1}{2}\cdot{}x + b_g$.

Nun für $f$ und $g$ den Punkt der Nullstelle $N=(5|0)$ einsetzen:

$f(5) = 0$ heißt $2\cdot{}5 + b_f = 0$ und somit ist $b_f=-10$.

Lösung für $f$:
$$f(x) = 2\cdot{}x - 10$$

$g(5) = 0$ heißt $\frac{-1}2 \cdot{}5 + b_g=0$ und somit ist
$b_g=\frac52$.

Lösung für $g$:
$$g(x) = \frac{-1}2\cdot{}x + \frac{5}{2}$$
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Drei Punkte Aufgabe}
...
$P\in h$ heißt: Der Punkt $P$ liegt auf dem Graphen der Geraden $h$.
...
\TNTeop{CHF 1.85 / km  $= 7.40 / 4 = (5+4\cdot{}0.6) / 4$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
