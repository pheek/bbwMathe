%% Layout for Kompendium
%% (c) Jan. 2026 philipp.freimann@bms-.ch

%%%%%%%%%%%%%%%  H E A D E R   &   F O O T E R %%%%%%%%%%%%%%%%%%%%
\fancyhf[HL]{Aufgabensammlung GESO} 
\fancyhf[HR]{BMS Winterthur (\texttt{https://www.bmwin.ch})}
\fancyhf[FR]{\leftmark}
\fancyhf[FL]{\thepage{}/\pageref{LastPage}}
\fancyhf[FC]{}


%% Einzug bei Paragraphen unterdrücken, damit alle Aufgabennummern
%% ganz links kommen
\setlength{\parindent}{0pt}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Layout erste Seite
\newcommand{\printKFirstPage}{%%
  \thispagestyle{empty}

\begin{center}{\huge{Diskussionsgrundlage}}\end{center}


\begin{tabular}{c}
%\raisebox{-10mm}{\includegraphics[width=30mm]{img/layout/loewe.pdf}} &
%{\huge{\textbf{Aufgabensammlung\hspace{-3mm}\,}}} 
%\raisebox{-10mm}{\includegraphics[width=30mm]{img/layout/bmsw.pdf}} 
\end{tabular}

\begin{center}{\huge{\textbf{Aufgabensammlung\hspace{-3mm}\,}}}  \end{center}

%  \begin{center} 
%\raisebox{-30mm}{\includegraphics[width=30mm]{img/layout/loewe.pdf}}

%\kKommentar{WORKING DRAFT} \end{center}

  \vspace{5mm}
  
  \begin{center} {\large{GESO}} (BMS Gesundheitlich-Soziale Richtung) \end{center}

  \kKommentar{KOMMENTIERTE VERSION (NICHT für SCHüLER/ERS/INNEN)}

  \begin{center}{\textbf{%% center und fett
    \isLoesungen{\deu{Lösungsteil}\eng{Solutions}}
    \isAufgaben{\deu{Aufgabenteil}\eng{Collection of Tasks}}
    }}%% fett
  \end{center}

  \vspace{5mm}

  \begin{center} \deu{---} \eng{english version} \end{center}

\begin{center} \auchTrainingsAufgabe{\deu{Angereichert mit
Einstiegsaufgaben/Trainingsaufgaben \,}\eng{Enhanced with simple
excerises.} \colorbox{blue!30}{(T)}} . \end{center}


  \vspace{10mm}
  \vspace{15mm}

  \begin{center}Version [\kVersion{}] Kanton Zürich \end{center}
  \begin{center}\kLehrplan\end{center}
\kKommentar{A.B.C}

\kKommentar{A=Version: Pro Jahr max eine Version, Kapitelstrukturänderung}

\kKommentar{B=Sub-Version: Formulierungen, Reihenfolgen etc.}

\kKommentar{C=Revision: Fehlerkorrektur}
%\kKommentar{Nur \textbf{eine} Version im Kanton?}

  
\newpage

\auchTrainingsAufgabe{%%

\begin{tabular}{c|p{18cm}}

{\colorbox{blue!30}{(T)}}  & 
 \deu{lediglich fürs Training, Einstiegsaufgaben}%% end deu
 \eng{only for training purpouse.}%% end eng
\\\hline

{\colorbox{red!30}{\,}} & Y
  \deu{Aufgaben mit Niveau der BMP (Berufsmaturitätsprüfung).}%% end deu
  \eng{level BMP (Berufsmaturitätsprüfung).}
%}%% end eng
\\
\end{tabular}

}%% end INKL Trainingsaufgaben

\kKommentar{Aufgabennummern ab Kapitel durchnummeriert}

\kKommentar{\hspace{5mm}startet je bei modulo 100 (bei 000 + 1)}

\kKommentar{Nutzungsidee angeben:}

(Wie ist das Kompendium zu verstehen?)

Worauf können sich die Lehrpersonen verlassen zur Prüfungsvorbereitung
und Jahresplanung? Was ist maßgeblich?
\begin{itemize}
\item \textbf{Kompendium!}
\item Rahmenlehrplan (\textbf{RLP} 2030)?
\item Alte Abschlussprüfungen?
\item Nachreiche-Blatt?
\item Nullserie?
\end{itemize}

Offene Fragen (eckige Klammern: Geschätzter Aufwand in Stunden):

\textbf{Weglassen (?):}

    Höhere Potenzgleichungen [0.5]\\
    Potenzfunktionen [0.5]\\
    Alle TR-Aufgaben (Gleichungssysteme vereinfachen, ABC-Formel vereinfachen) [1]\\
    Doppelbrüche [0.5]\\
    Kombination mit Wiederholung [0.5]\\
    Variation ohne Wiederholung [0.5]\\
    Substitution [1]\\
    Betrag [0.5]\\
    Betragsgleichungen (hat es noch im aktuellen Kompenidum) [0.5]\\
    Wurzelgleichung [0.5]\\
    Brüche mit negativen Exponenten kombiniert [1]\\
    Verschachtelte Wurzelterme [1]\\
    logarithmische Gleichungen (? oder kommen die gerade dazu?) [0.5-2]\\
    signifikante Stellen [0.5]\\
    Wissenschaftliche Notation [1] (? oder kommen die gerade dazu?)\\
    ... \\

\textbf{Neu hinzunehmen (?):}

    Lot auf eine Gerade durch einen gegebenen Punkt (ohne Abstand, denn das wäre eine Geometrie-Frage) [2]\\
    95\,\%-Prognose-Intervall [4]\\
    Signifikanzniveau [4]\\
    Sigma-Regel [4]\\
    Einseitiger, zweiseitiger Test [8]\\
    Nullhypothese [4]\\
    Fehler erster Art, Feher 2. Art [2-4]\\
    ...\\



\vspace{30mm}
\begin{center}\begin{small} \textbf{\deu{Autoren} \eng{Authors}}\\
Susanne Wagner (susanne.wagner@bms-zuerich.ch)\\
Urs Vonesch\\
Thomas Fellmann (thomas.fellmann@bms-zuerich.ch)\\
Mirjam Bräm, Wolfgang Pfalzgraf, Ulrike Gruber\\
Irene Schönenberger (irene.schoenenberger@edu.zh.ch)\\
Rita Franzetti (rita.franzetti@edu.zh.ch)\\
Christian Hersberger (christian.hersberger@bms-w.ch)\\
Philipp Freimann (philipp.freimann@bms-w.ch)\\
\vspace{2mm}

Nutzungsrechte: CC-NC-BY (2015-2026)
\kKommentar{ CC darf aus rechtlichen Gründen niemals entfernt werden!}

\end{small}%%
\end{center}


Quelltext des Skripts:\\
%%{\small \texttt{https://github.com/pheek/bbwMathe/tree/main/arbeitsblaetter/GESO\_Kompendium}}
{\small \texttt{
\href{https://github.com/pheek/bbwMathe/tree/main/arbeitsblaetter/GESO_Kompendium}{github}}(Quelltext Online)}


\newpage


\tableofcontents{}
\newpage
\pagestyle{fancy}
}%% end firstPage
