...
\kTrainingAufgabe{\eng{Multiply out and simplify as far as possible:}
...
\deu{Was ist jeweils die Definitionsmenge $\mathbb{D}$?}
\eng{What is the domain $\mathbb{D}$ in each case?}
...
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]  
  \item $\frac{2x^2-5x-12}{x^2+x-2}$  \deu{mit}\eng{width} $\mathbb{D} = \mathbb{R} \backslash \{-2; 1\}$
  \item $\frac{3x^2 -x -11}{x^2+X-2}$ \deu{mit}\eng{width} $\mathbb{D} = \mathbb{R} \backslash \{-2; 1\}$
  \end{enumerate}
\end{multicols}
}%% end aufgabe
