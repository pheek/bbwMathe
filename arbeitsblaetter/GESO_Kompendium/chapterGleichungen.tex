...
\kTrainingAufgabe{
\eng{Determine the solution set with respect to the universal set}\eng{Bestimmen Sie die Lösungsmenge bezüglich der
Grundmenge}
$G=\mathbb{R}$:
\\
...
züglich der Grundmenge}
\eng{Solve the systems of linear equations using an appropriate
solution method (addition, substitution, equating methods). Determine
...
\deu{Der Abschreibungssatz beträgt 17.5\%.}
...
\deu{Die gesuchte Zahl heisst 17.}
...
angeben:}
\eng{State the domain, then solve the equation, and determine the solution set; $G=\mathbb{R}$. Provide exact values.}
\\
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
...
Taschenrechner. $G=\mathbb{R}$.}
\eng{State the domain, then solve the equation, and determine the solution set using the calculator. $G=\mathbb{R}$.}
\\
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
...
\mathbb{R}$):}
\eng{Solve the equation for $x$ without using the calculator's solve function and express the results as exact values ($G = \mathbb{R}$):}
