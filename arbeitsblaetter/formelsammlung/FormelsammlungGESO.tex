...
%% Steuere später über das File "HECH.flag"
%%\isHECHtrue %% f(x) = ax+b
\isRLPtrue %%f(x) = mx+q
...
Produkt: $$2\cdot(x\cdot{}y) = 2(xy) = 2xy$$

\textbf{Aber}: Distributivgesetz: $2\cdot(x+y)=2x+2y$
...
\begin{tcolorbox}[colback=white]
\textbf{Gesetze}\TRAINER{ev. nur 3. Gesetz, andere sind kein Lernziel}
...
Steigung \fbox{\HECH{$a$}\RLP{$m$}$ = \frac{\Delta y}{\Delta x} = \frac{y_2-y_1}{x_2-x_1}$}.\\
...
\HECH{$$y=c + b\cdot{}a^x$$ oder
$$y=c + b\cdot{}e^{mx}$$}

\RLP{$$y=c + a\cdot{}q^x$$ oder
$$y=c + a\cdot{}e^{bx}$$}
...
\HECH{$$y=c+b\cdot{}a^\frac{x}\tau$$ oder
$$y=c+b\cdot{}e^{mx}$$}
\RLP{$$y=c+a\cdot{}q^\frac{x}\tau$$ oder
$$y=c+a\cdot{}e^{bx}$$}
...
\bbwCenterGraphic{18cm}{boxplotPhi.png}
...
«\missifarbig»-Formel: Sind einige Elemente gleich, so reduziert
...
\textbf{Achtung}: Das Gegenteil von «Ballspieler trifft minimal vier Mal»
...
\hline
\fbox{$\mu=\sum_{i=1}^n x_i\cdot{}p_i$} & $x_i$ = $i$-ter Wert der
