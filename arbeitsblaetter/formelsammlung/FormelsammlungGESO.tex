...
\usepackage{makecell}
\renewcommand{\metaHeaderLine}{Formelsammlung GESO (v 0.0.12 2021-06-14 fp)}
...
Bruchteil Arbeit pro Stunde = Leistung

Leistung = $\frac{1 \textrm{ [Arbeit]}}{x \textrm{ [Stunde]}}$

Physik: Leistung = $\frac{1 \textrm{ [Joule]}}{x \textrm{ [Sekunde]}} = [Watt]$
...
\item $p$ = Zinssatz (in \%); $f = 1+\frac{p}{100}$ = Zinsfaktor
...
\item
Tage rechnen bei vorgegebenem Wert (\zB $30m^2$):\\
...
\item {\color{orange} Median ($\mediantilde{x} = x_{\textrm{MED}}$)}, und Quartile ({\color{blue}$Q_1$},{\color{green}$Q_3$})einzeichnen
