...
\renewcommand{\metaHeaderLine}{Formelsammlung (v 0.0.28 DRAFT 2021-06-22 fp)}
\renewcommand{\arbeitsblattTitel}{Formelsammlung GeSo (working Draft)}
...
$$\sqrt[n]{\sqrt[m]{a}}   = \left((a)^\frac1m \right)^\frac1n = a^\frac1{nm}$$
...
$$\log_{\color{ForestGreen}a}({\color{red}b}) = \frac{\lg({\color{ForestGreen}a})}{\lg({\color{red}b})} = \frac{\ln({\color{ForestGreen}a})}{\ln({\color{red}b})}$$
\end{gesetz*}
...
${\color{red}b} : ={\color{orange}\frac1{y-2}}$:

\TRAINER{Formel zurückholen phi}
...
\subsubsection*{...Nullstelle} $x_0$ = Schnittstelle mit der $x$-Achse ($y$ null setzen und auf\/lösen).
...
\raisebox{-35mm}{\includegraphics[width=35mm]{img/tiprobuttonimages/ti30C.png}}
&
