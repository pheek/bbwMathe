...
\newcommand{\versionsnummerFoSa}{2021-06-26 0.1.8}
...
\TRAINER{Noch eine Graphik zu den Transformtaionen von Graphen?}
...
$$f(t) = b\cdot{}a^t$$
$b$ $\rightarrow$ Anfangswert bei $t=0$; $b=f(0)$\\
\phantom{$b$} = $y$-Achsenabschnitt\\
$a$ $\rightarrow$ Vervielfältigungsfaktor ($a\in\mathbb{R}^{+}$)\\\
$a>1$ $\rightarrow$ Wachstumsrate\\
$0<a<1$ $\rightarrow$ Zerfallsrate
\end{definition*}
...
\TRAINER{«Schritte zum Boxplot als Fließtext
  daneben. Ausreißerschwellen {\color{ForestGreen}grün} im Diagramm. Dann im Text auch}
Schritte zum Boxplot:\\
Fühler (Whisker) werden auf $1.5\times\textrm{IQR}$ begrenzt (= Ausreißerschwellen),
