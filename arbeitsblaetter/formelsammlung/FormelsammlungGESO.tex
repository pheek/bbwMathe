...
\newcommand{\versionsDatumFoSa}{2026-08-14}
\newcommand{\versionIdSubRev}{1.0.0}
...
\renewcommand{\metaHeaderLine}{Formelsammlung (RLP 2030)--- gesundheitlich-soziale Berufsmaturität}%% END HEADERFOOTER
\renewcommand{\geometrieErsteSeite}{\newgeometry{left=7mm,right=7mm,top=22mm,bottom=15mm}}

\definecolor{dunklesGruen}{HTML}{4E8F00}%%
...
\thispagestyle{fancy} \fancyhf[HC]{} \fancyhf[HRE]{} \fancyhf[HRO]{\thepage{}} \fancyhf[HLO]{} \fancyhf[HLE]{\thepage{}}%%
}%% END newcommand "headerUndFooterJedeSeite
...
\subsection*{Weitere Aufgabentypen} Mischaufgaben und Zahlenrätsel.

\subsection*{Vorgehen}

1. Problem in eigenen Worten vestehen

2. Unbekannte präzise definieren (inkl. Masseinheit)
...
\subsection*{Zeitvariable $x$}
%%$\e$ = Eulersche Konstante $\approx 2.71828$
\FAKTORBA{$$y=b\cdot{}a^x$$} %%entspricht $$y=b\cdot{}e^{mx}$$}

\FAKTORAQ{$$y=a\cdot{}q^x$$} %%entspricht $$y=a\cdot{}e^{bx}$$}
...
\FAKTORBA{$$y=b\cdot{}a^\frac{x}\tau$$} %% entspricht %$$y=b\cdot{}\e^{mx}$$}
\FAKTORAQ{$$y=a\cdot{}q^\frac{x}\tau$$} %% entspricht %$$y=a\cdot{}\e^{bx}$$}
...
\FAKTORBA{$\frac12 \cdot{} b = f(t) = b\cdot{}a^{\frac{t}{\tau}}\phantom{xxx}\Longrightarrow\phantom{xxx}T_{1/2}= \tau\cdot{}\log_a\left(\frac12\right)$}
\FAKTORAQ{$\frac12 \cdot{} a = f(t) = a\cdot{}q^{\frac{t}{\tau}}\phantom{xxx}\Longrightarrow\phantom{xxx}T_{1/2}= \tau\cdot{}\log_q\left(\frac12\right)$}
\else
\FAKTORBA{$\frac12 \cdot{} b = f(t) = b\cdot{}a^t\phantom{xxx}\Longrightarrow\phantom{xxx}T_{1/2}= \log_a\left(\frac12\right)$}
\FAKTORAQ{$\frac12 \cdot{} a = f(t) = a\cdot{}q^t\phantom{xxx}\Longrightarrow\phantom{xxx}T_{1/2}= \log_q\left(\frac12\right)$}
\fi
...
\ifisHALBWERTSZEITMITTAU%%
\FAKTORBA{$2\cdot{}b = f(t) = b\cdot{}a^{\frac{t}{\tau}}\phantom{xxx}\Longrightarrow\phantom{xxx}T_2 = \tau\cdot{}\log_a(2)$}
\FAKTORAQ{$2\cdot{}a = f(t) = a\cdot{}q^{\frac{t}{\tau}}\phantom{xxx}\Longrightarrow\phantom{xxx}T_2 = \tau\cdot{}\log_q(2)$}
\else
\FAKTORBA{$2\cdot{}b = f(t) = b\cdot{}a^t\phantom{xxx}\Longrightarrow\phantom{xxx}T_{2} = \log_a(2)$}
\FAKTORAQ{$2\cdot{}a = f(t) = a\cdot{}q^t\phantom{xxx}\Longrightarrow\phantom{xxx}T_2 = \log_q(2)$}
\fi
...
\item Das obige Histogramm ist unimodal, \textbf{linksschief} (und rechtssteil).
\end{itemize}
...
Einseitiger Hypothesentest mit vorgegebenem Signifikanzniveau.

\textbf{Gegeben}:

Trefferwahrscheinlichkeit pro Versuch: $p$ (Beispiel $p=0.7$)

Anzahl Versuche, Anzahl Beobachtungen: $n$ (Beispiel $n=30$)

Signifikanzniveau $\alpha$ (Beispiel $\alpha \le 10\,\%$)

\textbf{Hypothesen}

Nullhyphthese: 70\% «Treffer»

Typische Alternativhypothese: Wir \textbf{vermuten mehr} als 70\,\% «Treffer»


\textbf{Gesucht}:

Annahmebereich der Nullhypothese [0;k-1] bzw. Ablehnungsbereich [k; n]

Tabelle TR mit «BinomialCdf» (n=30, p=0.70)
...
\textit{Mit 92\,\% Wahrscheinlichkeit lande ich im Annahmebereich der Nullhypothese}

Ablehnungsbereich: 25..30 

\textit{Mit 7.66\,\% ($\le \alpha$) liegen wir bei 25 bis 30
  «Treffern» im Ablehnungsbereich der Nullhypothese
  und vermuten ``zu recht'', dass die Nullhypothese falsch war.}

\bbwCenterGraphic{160mm}{img/Binomialtest_FoSa_GeSo.pdf}

\textbf{Fehler:}

Fehler 1. Art: Ein in Realität faires Ergebnis wird als «gezinkt» taxiert.

Fehler 2. Art: Ein in Realität «gezinktes» Ergebnis wird als fair taxiert.
...
\raisebox{-45mm}{\includegraphics[width=45mm]{img/tiplusbuttonimages/ti30.png}}
&
