...
\BREITERRAHMEN{\renewcommand{\metaHeaderLine}{Formelsammlung (GESO \noTRAINER{2023}\TRAINER{\versionsnummerFoSa{}} BMS Winterthur)}}
\RAHMENLOS{\renewcommand{\metaHeaderLine}{}}

\newcommand{\geometrieErsteSeite}{\newgeometry{left=3mm,right=3mm,top=3mm,bottom=3mm}}

\BREITERRAHMEN{\renewcommand{\geometrieErsteSeite}{\newgeometry{left=7mm,right=7mm,top=7mm,bottom=7mm}}}
\RAHMENLOS{\renewcommand{\geometrieErsteSeite}{\newgeometry{left=3mm,right=3mm,top=3mm,bottom=3mm}}}


\newcommand{\headerUndFooterDieseSeite}{%%
\RAHMENLOS{\thispagestyle{empty}}%%
\BREITERRAHMEN{\thispagestyle{fancy}\fancyhf[FC]{Seite \thepage{}}}%%
}%%
...
\SCHWARZETITEL{
isswarte
\definecolor{titelFarbe}{rgb}{0,0,0}
}%%
...
\begin{tcolorbox}[colback=white]
\ALLELOGGESETZE{$$\log_a(u\cdot v) = \log_a(u) + \log_a(v)$$}
\ALLELOGGESETZE{$$\log_a(u : v) = \log_a(u) - \log_a(v)$$}

\FAKTORBA{$$\log_a(b^x) = x\cdot{}\log_a(b)$$
$$\log_{\color{farnFarbe}a}({\color{red}b}) = \frac{\lg({\color{red}b})}{\lg({\color{farnFarbe}a})} = \frac{\ln({\color{red}b})}{\ln({\color{farnFarbe}a})}$$}
\FAKTORAQ{$$\log_b(a^x) = x\cdot{}\log_b(a)$$
...
\bbwCenterGraphic{17cm}{img/KombinatorikHECH.pdf}
