...
\newcommand{\versionsnummerFoSa}{2021-06-28 0.2.0}
...
%%$x^a=c$ & $\Leftrightarrow$ & $x=\sqrt[a]{c}$\\
  \TRAINER{$x^a = c$ und $x^\frac1b =c$ sind darin enthalten.
...
\TRAINER{ev. in der Grafik $m$ durch $ab$ ersetzen.

}
Wachstum: $a>1$\\
Beim Zerfall gilt: $0<a<1$
...
%und Quartile ({\color{blue}$Q_1$}, {\color{ForestGreen}$Q_3$})einzeichnen
%\item IQR := {\color{ForestGreen}Q3}-{\color{blue}Q1} rechnen
...
«Mississippi»-Formel: Wie viele Wörter sind mit den Buchstaben des Wortes «Mississippi» bildbar? Sind einige Bucchstaben gleich, so reduziert sich die Anzahl der Möglichkeiten.
$$N = \frac{11!}{4!4!2!}$$
