...
%% Für Schreibweise mit a,b,c (Hersberger Christian)
\newif\ifisHECH
\newif\ifisRLP
...
Erst in \textbf{Grundform} bringen:
...
\HECH{$$g\-:\,\,\,\,\, y = f(x) = a\cdot{}x + b$$}
\RLP{$$g\-:\,\,\,\,\, y = f(x) = m\cdot{}x + q$$}

\RLP{\TRAINER{Formel in Grafik ersetzen durch $mx+q$}}
...
\begin{tabular}{cc}
$z$ > 0: Parabel ($z$ gerade) & $z$ > 0: Parabel ($z$ ungerade)\\   %%\bbwCenterGraphic{8cm}{PotenzFkt.pdf}
...
%% \includegraphics[width=4cm]{allg/funktionen/img/potenzfct/potenzFunktionenGerade.png}\hfill{}\includegraphics[width=4cm]{allg/funktionen/img/potenzfct/potenzFunktionenUngerade.png}
  ...
  Der Faktor $a$ in $y=a\cdot{}x^k$ streckt die Funktion in $y$-Richtung, bzw. spiegelt die Funktion an der $x$-Achse.
...
\HECH{\bbwCenterGraphic{8cm}{ExpFktVergleichMitLinearemWachstum.pdf}}
...
\HECH{$$y=c + b\cdot{}a^x$$ entspricht
$$y=c + b\cdot{}e^{mx}$$}

\RLP{$$y=c + a\cdot{}q^x$$ entspricht
$$y=c + a\cdot{}e^{bx}$$}
...
\HECH{$$y=c+b\cdot{}a^\frac{x}\tau$$ entspricht
$$y=c+b\cdot{}e^{mx}$$}
\RLP{$$y=c+a\cdot{}q^\frac{x}\tau$$ entspricht
$$y=c+a\cdot{}e^{bx}$$}

\HECH{$\textrm{mit } m=\ln\left(a^\frac1\tau\right) = \frac1\tau \cdot{}\ln(a) \,\,\textrm{ oder } a = e^{m\tau}$}
\RLP{$\textrm{mit } b=\ln\left(q^\frac1\tau\right) = \frac1\tau \cdot{}\ln(q) \,\,\textrm{ oder } q = e^{b\tau}$}
...
\RLP{\bbwCenterGraphic{18cm}{boxplotPhi.png}}
\HECH{\bbwCenterGraphic{18cm}{boxplot4.png}}
...
Dies kann für auch mit einem Baumdiagramm gelöst werden.
...
«\missifarbig»-Formel: Sind bei einer Permutation (Vertauschung) einige Elemente gleich, so reduziert
...
\TRAINER{nCr in der Grafik raus, da bei TR. Dafür immer $N=$ vor die Formeln.}
\RLP{\bbwCenterGraphic{17cm}{img/Kombinatorik2.pdf}}
\HECH{\bbwCenterGraphic{17cm}{img/Kombinatorik.pdf}}
...
wahrscheinlich.
Dies entspricht in obiger Tabelle «mit Zurücklegen».
...
($p=0.35$). Wie groß ist die Wahrscheinlichkeit, dass er mit 20 Würfen
($n=20$) genau vier Mal ($k=4$) trifft?
...
Zufallsgröße. $p_i$ = Wahrscheinlichkeit  $P(X=x_i)$\\
 \hline
 \end{tabular}
...
\tiprobutton{math}         &math              & Summenzeichen $\Sigma$ (\tiprobutton{5})\\
                           &math              & Faktorzerlegung (\tiprobutton{4}\texttt{Pfactor})\\\hline
...
\tiprobutton{table}        & table            &  Wertetabelle für diverse Funktionen: Lineare Fct., Potenzfct., Exponentialfct.\\\hline
\tiprobutton{ncrnpr}       &Kombinatorik      &  Fakultät: {\color{red}\textbf{!}}\\
                           &                  &  $n \choose k$: Tippe $n$ nCr $k$\\
                           &                  &  $\left(\frac{n!}{(n-k)!}\right)$: Tippe $n$ nPr $k$\\\hline
