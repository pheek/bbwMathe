...
$g_2$: $M=(-4|9|0)$ und Mittelpunkt zwischen $A$ und $S$ ist gleich $(-2|7|3)$.
...
\begin{itemize}
\item zusammenfallend oder parallel mit einer Achse
\item liegt in einer Ebene oder ist parallel zu einer Ebene ($xy$-,
$xz$- oder $yz$-Ebene)
\end{itemize}
...
Analysieren Sie die gegenseitige Lage je zweier Geraden (Tipp:
Skizzieren Sie alle Geraden in einem Koordinatensystem):
\TNTeop{
$g$ und $h$: schneidend
...
= \frac13\cdot{}\sqrt{25+64+1} = \sqrt{10} \approx 3.162278$$
...
Beispielhaft wird Aufgabe a) vorgelöst:

a)
 $$\vec{s} = \vec{p} - \vec{a} - t\cdot{}\vec{v} = \Spvek{4t-3;5-3t}$$
 
  $$0=\vec{s}\circ{}\vec{v} = (4t-3)\cdot{}(-4) + (5-3t)\cdot{}3 \Longrightarrow $$

  $t=\frac{27}{25}$

 
  $|\vec{s}| = \left| \Spvek{\frac{33}{25}; \frac{44}{25}}\right| = 2.2 = \frac{11}5$
...
(Parameter $s$ und $t$ gehen hier ganzzahlig auf.)

\TNT{16}{
Siehe Lösungen OLAT Marthaler S. 307 Aufg. 35

a) $\overrightarrow{PQ}= \Spvek{3t+s+3;-5t+s+9;-s-t+2}$; Parameter: $t=1$ und $s=-3$; Distanzvektor $\overrightarrow{PQ} = \Spvek{3;1;4}$; Abstand = $\sqrt{26} \approx5.099$
}
\noTRAINER{\newpage}
...
b) $\overrightarrow{PQ} = \Spvek{-3s+t+4;-s-8;-s+t+6}$;  Parameter: $t=\frac{-35}3$ und $s=\frac{-10}3$; Distanzvektor $\overrightarrow{PQ} = \frac73\cdot{}\Spvek{1;2;1}$; Abstand = $\frac37 \cdot{} \sqrt{6} \approx 5.72$
}
}%% end AufgabenNr
