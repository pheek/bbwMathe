...
a) $\overrightarrow{OA}$,\\
b) $\overrightarrow{BA}$ und\\
c) $\overrightarrow{OA} + 3\cdot{} \overrightarrow{AB}$
}{

a) $$\overrightarrow{OA} = \Spvek{7;5.5;-3}$$
b) $$\overrightarrow{BA} = \Spvek{7;5.5;-3} - \Spvek{5;-2;0} = \Spvek{2;7.5;-3}$$
c)
$$\overrightarrow{OA} + 3\cdot{} \overrightarrow{AB} =
\Spvek{7;5.5;-3} + 3\cdot{} \Spvek{-2;-7.5;3} =
\Spvek{7-3\cdot{}(-2); 5.5+3\cdot{}(-7.5); -3 + 3\cdot{}3} =
\Spvek{13;-17;6}$$
}
...
b) Berechnen Sie die Länge des folgenden Vektors ($\vec{b}$):
...
\TRAINER{
$$\overrightarrow{BA} = \Spvek{7-(-4);2-3;6-(-1)} = \Spvek{11;-1;7}$$
$$\overrightarrow{CB} = \Spvek{-4-0; 3-0; -1-8}   = \Spvek{-4;3;-9}$$
$$\overrightarrow{CA} = \Spvek{7-0;2-0;6-8}       = \Spvek{7;2;-2}$$
}%% end TRAINER
...
solve(norm([-4,3,12]-[x,0,0]) = 3x norm([16,2,-1]-[x, 0, 0]),x)

$$\lx= \frac{37\pm\sqrt{31}}{2}$$
$$\lx \approx \{10.15; 26.85\}$$
}%% end TRANIER
}%% end aufgabeML
\newpage
...
\aufgabeML{Berechnen Sie $\vec{b}$ so, dass $\vec{b}$ den Betrag 1 hat
und dem Vektor $\vec{a}$ entgegengesetzt ist:
$$\vec{a} = \Spvek{3;0;-4}$$

\vspace{12mm}
}{
Entgegengesetzt: $\Spvek{-3;0;4}$

Länge von $\vec{a} = \sqrt{(-3)^2 + 4^2} = \sqrt{9+16}= \sqrt{25} = 5$

$$\vec{b} = \frac15 \cdot{} \Spvek{-3;0;4} = \Spvek{-0.6;0;0.8}$$}%% end AufgabeML
\newpage
...
Dieses $\lambda$ setzen wir in die erste Komponente ein und
...
\newpage
\aufgabeML{Bestimmen Sie den Parameter $k$ so, dass es sich beim
folgenden um einen Einheitsvektor handelt:
$$\Spvek{0.7; k; \frac1{10}}$$
(Mit Taschenrechner auf 3 signifikante Stellen angeben.)


}{Lösung = \LoesungsRaumLang{$k=\pm \frac{\sqrt{2}}{2} \approx \pm 0.707$}}
...
Gegeben seien die Vektoren $\vec{a}$, $\vec{b}$ und
$\vec{c}$.
...
}{$$\vec{d}  = \LoesungsRaumLang{-\vec{a} -\frac{13}{20}\cdot{} \vec{b} + \frac25\cdotp{}\vec{c}}$$}


\aufgabeML{(mit TR)

Stellen Sie den Vektor $\vec{z}$ als Linearkombination von
...
\vspace{12mm}
}{$$\vec{z} = \LoesungsRaum{\frac{-552}{49}}\cdot{}\vec{a}
+ \LoesungsRaum{\frac{-864}{49}}\cdot{}\vec{b} + \LoesungsRaum{\frac{1356}{49}}\cdot{}\vec{c}$$}
\newpage


\aufgabeML{(von Hand)

Lässt sich der Vektor $\vec{a}$ als Linearkombination von
...
Gegeben sind die Eckpunkte eines Dreiecks:
 $A=(7|7|7)$, $B=(8|8|6)$ und $C=(3|4|-1)$

Gesucht ist die Länge der Winkelhalbierenden
$\left| \overrightarrow{w_\gamma} \right| = w_\gamma$

Tipp: Bestimmen Sie zunächst von den Seiten $a$ und $b$ normierte
Vektoren $\overrightarrow{n_a}$ und $\overrightarrow{n_b}$ (Länge
1). Dann liegt der Vektor $\overrightarrow{n_a}
+ \overrightarrow{n_b}$ definitiv auf der Winkelhalbierenden des
Winkels $\gamma$.

Auf den Schnittpunkt der Winkelhalbierenden mit der Seite $c$ gelangen
wir nun auf zwei Wege: 1. entlang der Winkelhalbierenden, 2. entlang
der Seite $c$ von $A$ oder $B$ aus.


}{%% Lösung
$\overrightarrow{n_a} := \frac{\vec{a}}{a} = \frac{\overrightarrow{CB}}{|\vec{a}|}$

$\overrightarrow{n_b} := \frac{\vec{b}}{b} = \frac{\overrightarrow{CA}}{|\vec{b}|}$

Für ein $i$ und ein $k$ gilt:

$\overrightarrow{w_\gamma} = i\cdot{} (\overrightarrow{n_a}
+ \overrightarrow{n_b}) = \vec{a} + k\cdot{}(\overrightarrow{BA})$

Zudem:

$$\vec{a} = \overrightarrow{CB} = \Spvek{5;4;7}; a = \sqrt{90}$$
$$\vec{b} = \overrightarrow{CA} = \Spvek{4;3;8}; b = \sqrt{89}$$
$$\vec{c} = \vec{b} - \vec{a} = \Spvek{-1; -1; 1}$$


$$\overrightarrow{n_a} = \frac{1}{\sqrt{90}} \cdot{} \Spvek{5;4;7}$$
$$\overrightarrow{n_b} = \frac{1}{\sqrt{89}} \cdot{} \Spvek{4;3;8}$$
$$\overrightarrow{n_a} + \overrightarrow{n_b} =
\Spvek{\frac{5}{\sqrt{90}} + \frac{4}{\sqrt{89}}; \frac{4}{\sqrt{90}} + \frac{3}{\sqrt{89}}; \frac{7}{\sqrt{90}} + \frac{8}{\sqrt{89}}}$$

Mit den beiden «Wegen» ab Punkt $C$ zur gegenüberliegenden Seite gilt:

$$i\cdot{} (\overrightarrow{n_a}
+ \overrightarrow{n_b}) = \vec{a} + k\cdot{}(\overrightarrow{BA})$$
$$i\cdot{}\Spvek{\frac{5}{\sqrt{90}} + \frac{4}{\sqrt{89}}; \frac{4}{\sqrt{90}} + \frac{3}{\sqrt{89}}; \frac{7}{\sqrt{90}} + \frac{8}{\sqrt{89}}} = \Spvek{5-k; 4-k; 7-k}$$

Zwei dieser drei Gleichungen mit TR nach $i$ und $k$ aufgelöst ergeben
%$i=90\cdot{}\sqrt{89} -267\cdot{}\sqrt{10}$ und $k=90-3\cdot{}\sqrt{890}$.

Somit ist

$$\overrightarrow{w_\gamma} = \vec{a} + k\cdot{}\vec{c} = \Spvek{5;4;7} + (90-3\cdot{}\sqrt{890})\cdot{}\Spvek{-1;-1;1} = \Spvek{3\cdot{}\sqrt{890} -85; 3\cdot{}\sqrt{890} - 86; 97 - 3\cdot{}\sqrt{890}}$$

ergo
$$|\overrightarrow{w_\gamma}| = w_\gamma = \text{norm}\left({\overrightarrow{w_\gamma}}\right) \approx 9.4206$$
}%% end aufgabeML
